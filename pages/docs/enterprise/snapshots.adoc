...
* If restoring from an network file system (NFS) or a host path storage destination, see <<Network file system (NFS)>> or a <<Configuring a host path>> for the configuration steps and how to set up Velero. Otherwise, see the Velero documention for https://velero.io/docs/v1.5/basic-install/[installing] and https://velero.io/plugins/[configuring] the plugins.
+
