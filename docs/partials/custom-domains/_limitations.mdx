...
- Multiple custom domains can be active at once, but only one custom domain can be the default for each Replicated endpoint. All active custom domains work whether or not they are the default.