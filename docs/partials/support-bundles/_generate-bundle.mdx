Support bundles are generated from one or more support bundle specifications. Specification locations include YAML manifest files in the cluster, URLs, stdin, and Secrets or ConfigMap objects embedded in a Kubernetes cluster. For more information, see [Collecting a Support Bundle](https://troubleshoot.sh/docs/support-bundle/collecting/) and [Discover Cluster Specs](https://troubleshoot.sh/docs/support-bundle/discover-cluster-specs/) in the Troubleshoot documentation.
...
- Automatically discover specifications found in the cluster that match a custom label:
  ...
  ```bash
  kubectl support-bundle --load-cluster-specs -l troubleshoot.io/kind=LABEL
  ```
