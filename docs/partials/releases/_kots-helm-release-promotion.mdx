* A release that does not contain the required Kubernetes manifests for KOTS installations cannot be promoted to a channel where KOTS-enabled customers are assigned. For more information about the required Kubernetes manifests, see [Custom Resource Checklist](/vendor/distributing-workflow#custom-resources-checklist) in _Onboarding with KOTS_.
