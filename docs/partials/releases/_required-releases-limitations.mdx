Required releases have the following limitations:
  ...
  * If you change the channel an existing customer is assigned, the admin console always fetches the latest release on the new channel, regardless of any required releases on the channel. For more information, see [Channel Assignment](licenses-about#channel-assignment) in _About Customers_.