        ...
        - runPod:
            ...
            podSpec:
              ...
              - name: postgres-connection
                ...
                env:
                - name: PGHOST
                  value: '{{ .Values.global.externalDatabase.host }}'
                - name: PGUSER
                  value: '{{ .Values.global.externalDatabase.user }}'
                - name: PGDATABASE
                  value: '{{ .Values.global.externalDatabase.database }}'
                # the spec contains only the Secret name and key, not the password
                - name: PGPASSWORD
                  valueFrom:
                    secretKeyRef:
                      name: '{{ .Values.global.externalDatabase.existingSecret }}'
                      key: password
      analyzers:
        - textAnalyze:
