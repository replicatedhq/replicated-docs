...
When creating a token, users can choose to limit the service account to a read-only version of the selected RBAC policy. Names for service accounts must be unique within a given team.
