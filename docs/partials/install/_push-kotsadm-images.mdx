Extract container images from the `kotsadm.tar.gz` bundle and push the KOTS admin console images to your private registry:
    ...
    * `REGISTRY_HOST` with the hostname for the private registry. For example, `private.registry.host` or `my-registry.example.com/namespace/`.
