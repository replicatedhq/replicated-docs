In the example below, the `mlflow_ingress_type` field is displayed on the **Config** page only when the distribution of the cluster is _not_ [Replicated embedded cluster](/vendor/embedded-overview). This ensures that only users deploying to their own existing cluster are able to select an ingress type.
