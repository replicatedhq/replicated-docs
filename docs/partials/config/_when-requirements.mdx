...
* `when` cannot be applied to the items nested under a `select_one` item. To conditionally show or hide `select_one` items, apply the `when` property to the `select_one` item itself.