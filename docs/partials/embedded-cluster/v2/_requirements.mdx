...
* (SELinux hosts only) On a host with SELinux enabled, a Red Hat Enterprise Linux-family distribution with the `container-selinux`, `policycoreutils`, and `policycoreutils-python-utils` packages installed. For more information, see [SELinux](/embedded-cluster/v2/installing-embedded-selinux).
