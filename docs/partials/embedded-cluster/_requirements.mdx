...
* The host’s hostname must comply with [RFC 1123](https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#dns-subdomain-names): it must consist of lowercase alphanumeric characters (`a–z`, `0–9`), `-` or `.`, and must start and end with an alphanumeric character (e.g. `node-01`, `node.example.com`).
