...
For single-node installations, ensure that there are no other processes using these ports. Although there is no communication between nodes in single-node installations, having these ports open and available ensures that Embedded Cluster preflight checks pass.
