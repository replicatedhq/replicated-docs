1. Get the join command from the Admin Console:
   ...
   1. In the **Add a Node** dialog, select one or more roles for this node. If no custom roles are defined, the role selection will not appear and only the join command is displayed.
...
1. Repeat the process each node you want to add.