...
The Replicated SDK reads its configuration from Helm values only and does not support reading from Kubernetes Secrets or CSI-mounted volumes. Any values used by the SDK must be set in the chart's `values.yaml`, with the `--set` flag during deployment, or in a values file from a GitOps/CI workflow.
