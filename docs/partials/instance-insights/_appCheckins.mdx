* For Helm installations that include the Replicated SDK, the SDK automatically sends data to the vendor portal every four hours.

* The instance checks for available updates:
  * By default, KOTS checks for updates every four hours.
  ...
  * For Helm installations that include the Replicated SDK, an update check occurs when the instance makes a request to the `/api/v1/app/updates` SDK API endpoint. See [app](/reference/replicated-sdk-apis#app) in _Replicated SDK API (Alpha)_.
