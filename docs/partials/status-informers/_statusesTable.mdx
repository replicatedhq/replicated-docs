<table>
 <tbody>
    ...
    <tr>
      <th>Unknown</th>
      <td colSpan="6">Unknown is a special deployment status that indicates that no data points exist for a queried time period. An Unknown status might indicate that an instance has shut down and stopped checking in. Alternatively, an Unknown status might indicate that you queried instance data for a time period that begins and ends after the instance's last checkin with the <a href="/reference/vendor-api-using">Vendor API</a>. By default, instances check in every 4 hours. For more information about instance checkins, see <a href="/vendor/instance-insights-event-data#about-reporting">How the Vendor Portal Collects Instance Data</a> in <em>About Instance and Event Data</em>.</td>
    </tr>
  </tbody>
</table>
