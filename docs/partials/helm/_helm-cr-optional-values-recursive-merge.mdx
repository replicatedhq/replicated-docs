...
The admin console uses the values from this merged dataset and from the Helm chart `values.yaml` file when deploying the application. For an example of recursive merge, see [How KOTS Merges the `values` and `optionalValues` Datasets](/vendor/helm-optional-value-keys#example-recursive-merge) in _Manipulating Helm Chart Values with KOTS_.        
