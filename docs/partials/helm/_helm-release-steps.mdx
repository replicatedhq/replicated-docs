The following steps demonstrate how to create and promote a release using the vendor portal. You can also create and promote releases with the replicated CLI.
...
1. Click **Save Release** then click **Promote**. In the **Promote Release** dialog, edit the following fields:
   ...
   1. For **Version label**, the vendor portal automatically populates this field with the version in your Helm chart `Chart.yaml` file. If you have multiple Helm charts in your release, the vendor portal chooses the version from one of the `Chart.yaml` files. The version label must match the version in one of the Helm charts in your release.
   1. If the **Prevent this release from being skipped during upgrades** option is displayed, leave it unselected. This option applies only to releases installed with KOTS. For more information about this option for KOTS installations, see [Create a Release](/vendor/releases-creating-releases#create-a-release) in _Managing Releases in the Vendor Portal_. 
