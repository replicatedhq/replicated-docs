...
1. Click **Save Release** then click **Promote**. In the **Promote Release** dialog, edit the following fields:
   ...
   1. For **Version label**, the vendor portal automatically populates this field with the version in your Helm chart `Chart.yaml` file. If you have multiple Helm charts in your release, the vendor portal chooses the version from one of the `Chart.yaml` files. The version label must match the version of one of the Helm charts in your release.
