...
This topic describes how to configure automatic updates for application installed in online (internet-connected) environments. The information in this topic applies to Replicated KOTS installations in existing clusters, Replicated Embedded Cluster installations, and Replicated kURL installations.
...
For installations with Replicated Embedded Cluster, both the application and the cluster are updated together during automatic updates.
