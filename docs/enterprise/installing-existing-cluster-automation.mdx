...
Air gap installations in existing clusters require the application `.airgap` bundle and the `kotsadm.tar.gz` air gap bundle. Additionally, the application and KOTS admin console images must be pushed a local private registry.
...
1. Install the application:
      ...
      * `READ_WRITE_USERNAME` and `READ_WRITE_PASSWORD` with credentials that have read write permissions to the private registry where you pushed the images.
