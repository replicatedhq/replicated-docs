...
Private registries are required for air gap installations. During the air gap installation, you push the admin console container images to a private registry.

For online environments, it is optional to use private registries, which can be configured at any time after you install the application. For more information, see [Using Private Registries](image-registry-settings).
