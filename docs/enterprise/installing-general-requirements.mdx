...
## Velero Version Compatibility for Snapshots

The following table lists the versions of Velero that are compatible with each version of KOTS. Velero is required to enable the Replicated snapshots feature for backup and restore. For more information, see [About Backup and Restore](/enterise-snapshots-understanding).
...
* **Admin console minimum requirements**: Existing clusters that have LimitRanges specified must support these minium values for the admin console:

  * The admin console requires a minimum of 5GB of disk space on the cluster. This includes:

    * 4GB for the object store PersistentVolume 

      :::note
      You can optionally install KOTS without object storage. For more information, see [Installing Without Object Storage](installing-stateful-component-requirements).
      :::

    * 1GB for the rqlite PersistentVolume

  * The admin console pod requests 100m CPU resources and 100Mi memory.
