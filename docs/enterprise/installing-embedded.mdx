...
To relocate the Admin Console and LAM ports during installation with Embedded Cluster, you can use the `--admin-console-port` and `--local-artifact-mirror` flags with the Embedded Cluster `install` command.
...
The default Embedded Cluster host preflights check for the default ports (or the supplied ports), so users will be alerted if they need to choose new port(s).

### Limtiation

On restore, the Admin Console port will be used from the backup. That can't be changed. For LAM, the port from the backup will be used unless the user specifies a new port with the `restore` command. For example, `restore --local-artifact-mirror-port=50000`.