import RestoreType from "../partials/snapshots/_restore-types.mdx"
...
- Only full backups that include both the application and the admin console are usable in disaster recovery scenarios.

- Partial backups that include the application only _cannot_ be restored to a new cluster. Only full backups can be restored to a new cluster.
...
Partial backups can be useful if you need to roll back after deploying a new application version. Partial backups of the application only _cannot_ be restored to a new cluster, and are therefore are not useable for disaster recovery scenarios.
