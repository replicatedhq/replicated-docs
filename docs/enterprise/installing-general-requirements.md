...
* For domains owned by Replicated, a link to the list of required IP addresses for the domain
...
|--------|-------------------|-------------------|-------------|
| Docker Hub    | Required    | Required    | Some dependencies of KOTS are hosted as public images in Docker Hub. The IP addresses listed at [Allowlist for Docker Desktop](https://docs.docker.com/desktop/allow-list/) are required. |
...
| `registry.replicated.com` | Required&#42;&#42;         | Required&#42;&#42;            | Some applications host private images in the Replicated registry at this domain. The on-prem docker client uses a license ID to authenticate to `registry.replicated.com`. This domain is owned by Replicated, Inc which is headquartered in Los Angeles, CA. For a list of IP addresses for `registry.replicated.com`, see [replicatedhq/ips](https://github.com/replicatedhq/ips/blob/main/ip_addresses.json#L20-L25) in GitHub.
