...
1. In the **Embedded Cluster install instructions** dialog, enable the **Install in an air gap environment** checkbox. This appends the `?airgap=true` query parameter to the end of the `replicated.app` endpoint in the curl command in the first step, which ensures that the air gap bundle is included in the download.
