...
1. On each node in the cluster, download the kURL `.tar.gz` air gap bundle for the updated spec and extract the contents. For more information, see [Download the kURL Bundle](/vendor/releases-download-airgap-bundles#installer-bundle) in _Downloading Air Gap Bundles_.
