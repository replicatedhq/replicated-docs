...
1. Download the target version of the following assets from the download portal:
   * KOTS Admin Console `kotsadm.tar.gz` bundle
   * KOTS CLI plugin

   For more information, see [Download Air Gap Bundles from the Download Portal](/vendor/releases-download-airgap-bundles#download-portal). Ensure that you can access the downloaded bundle from the environment where the Admin Console is running.

   :::note
   The Admin Console and the CLI must be the same version.
   :::
