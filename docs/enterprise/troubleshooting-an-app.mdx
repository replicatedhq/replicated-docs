...
You can generate a support bundle using the support-bundle CLI instead of the admin console. For example, the admin console might not be available if you are using a Kubernetes installer (embedded installer) and the installation fails. Or perhaps you need to use a more recent version of the support-bundle plugin than what is embedded in the admin console.

### Prerequisite: Install the Plugin {#plugin}

<InstallPlugin/>

### Generate a Bundle (App Manager v1.94.2 and Later)

:::note
To generate support bundles on earlier versions of the app manager, see [Generate a Bundle (App Manager v1.94.1 and Earlier)](#earlier-versions).
:::

<GenerateBundle/>

### Generate a Bundle (App Manager v1.94.1 and Earlier){#earlier-versions}
