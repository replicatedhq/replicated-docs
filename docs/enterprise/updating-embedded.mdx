...
When you update an application installed with Embedded Cluster, you update both the application and the cluster infrastructure together, including Kubernetes, KOTS, Velero, and other components deployed to the cluster by Embedded Cluster. There is no need or mechanism to update the infrastructure on its own.
