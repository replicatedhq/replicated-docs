...
1. (Optional) In the Embedded Cluster Config, configure the `roles` key to define custom node roles. Custom node roles can be useful if you want to assign certain workloads to run on specific nodes. For more information, see [roles](/reference/embedded-config#roles) in _Embedded Cluster Config_. When you are done, create and promote a new release with the updated Config.
...
1. Do one of the following to get the join command from the Admin Console:
   ...
   1. Otherwise, if you have already installed the application:
         ...
         1. If you promoted a new release that defines the `roles` key in the Embedded Cluster Config, update the instance to the new version. See [Performing Updates in Embedded Clusters](/enterprise/updating-embedded).
...
High availability has the following requirements:

* High availability is supported with Embedded Cluster 1.4.1 or later.

* High availability is supported only for clusters where at least three controller nodes are present.
...
Consider the following best practices and recommendations for highly-available clusters:

* At least three _controller_ nodes that run the Kubernetes control plane are required for high availability. This is because clusters use a quorum system, in which more than half the nodes must be up and reachable. In clusters with three controller nodes, the Kubernetes control plane can continue to operate if one node fails because a quorum can still be reached by the remaining two nodes.

     By default, with Embedded Cluster, all new nodes added to a cluster are controller nodes, which run the control plane and can also run other workloads. If you plan to use three nodes in cluster and you choose to define node roles in the Embedded Cluster Config, consider defining `controller` roles, which can be assigned to controller nodes, rather than `custom` nodes, which can be assigned to worker nodes. For more information about defining roles for controller and worker nodes, see [roles](/reference/embedded-config#roles) in _Emebdded Cluster Config_.

* Always use an odd number of nodes in the cluster. Using an odd number of nodes ensures that the cluster can make decisions efficiently with quorum calculations. Clusters with an odd number of nodes also avoid split-brain scenarios where the cluster runs as two, independent groups of nodes, resulting in inconsistencies and conflicts.
