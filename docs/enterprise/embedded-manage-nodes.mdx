...
1. On a node that has internet access, copy and run the first command to download the Embedded Cluster installation assets (including the Embedded Cluster binary and the air gap bundle).

1. Move the downloaded assets to the air-gapped node that you want to join.

1. Untar the Embedded Cluster assets on the node.
...
1. On a node that is already joined to the cluster, run the following command to get the commands for adding a new controller node:
   ...
   The output of `get join-command` includes the commands for downloading the Embedded Cluster binary, extracting the binary, and joining the node to the cluster.
