...
* More than one controller node should not be joined at the same time. When joining a controller node, a warning is printed to say the user shouldn't join another node until the controller node joins successfully.
