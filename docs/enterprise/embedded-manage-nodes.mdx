...
1. (Optional) In the Embedded Cluster Config, configure the `roles` key to define node roles. Defining node roles can be useful if you want to assign certain workloads to run on specific nodes. For more information, see [roles](/reference/embedded-config#roles) in _Embedded Cluster Config_. When you are done, create and promote a new release with the updated Config.
