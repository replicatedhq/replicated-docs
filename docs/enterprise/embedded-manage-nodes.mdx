...
In most cases, users will add nodes to a cluster during installation. It is also possible to join nodes after installing. For information about joining nodes during installation, see [Online Installation with Embedded Cluster](/enterprise/installing-embedded) or [Air Gap Installation with Embedded Cluster](/enterprise/installing-embedded-air-gap).
...
1. In the Embedded Cluster Config, configure the `roles` key to define node roles. Node roles determine which nodes run the Kubernetes control plane and allow you to assign application workloads to particular nodes. For more information, see [roles](/reference/embedded-config#roles) in _Embedded Cluster Config_. When you are done, create and promote a new release with the updated Config.

1. Do one of the following to get the join command from the Admin Console:

   1. If you have not yet installed the application, follow the steps in [Online Installation with Embedded Cluster](/enterprise/installing-embedded) or [Air Gap Installation with Embedded Cluster](/enterprise/installing-embedded-air-gap) to install. A **Nodes** screen displays as part of the installation flow in the Admin Console.

   1. Otherwise, log in to the Admin Console. Update the instance to the new release that you promoted that defines the `roles` key in the Embedded Cluster Config. For more information, see [Performing Updates in Embedded Clusters](/enterprise/updating-embedded). Then, go to **Cluster Management > Add node** at the top of the page.
   
1. Either on the Admin Console **Nodes** screen or in the **Add a Node** dialog, select one or more roles for the new node that you will join.

     :::note
     If no [roles](/reference/embedded-config#roles) are defined in the Embedded Cluster Config, the role selection will not appear and only the join command is displayed.
     :::

     The following shows an example of the **Add a node** dialog:
...
1. Do one of the following to download and untar the Embedded Cluster installation assets on the machine that you will join to the cluster:
...
1. On the machine that you will join to the cluster, run the join command that you copied.
