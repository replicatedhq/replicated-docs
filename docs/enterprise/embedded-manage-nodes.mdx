...
1. Do one of the following:
   ...
   1. Otherwise, to add a node to an existing cluster:
        ...
        :::note
        You can also use the `sudo ./APP_SLUG join print-command` command to get the join command. See [Automate Node Joins](#automate-node-joins) below.
        :::         
...
1. If you are adding a third controller node to the cluster, respond to the prompt asking if you want to enable high availability. For more information, see [Create a Multi-Node Cluster with High Availability](#create-ha) below.
...
## Automate Node Joins (Beta) {#automate-node-joins}
...
To automate node joins:

1. On a node that is already joined to the cluster, run the `sudo ./APP_SLUG join print-command` command to get the join command for adding a node with the controller role.

1. On the node that you want to join, run the join command that was provided in the output of the `join print-command` command.
