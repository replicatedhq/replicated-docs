...
* Improves the default host support bundle generated by `./APP_SLUG support-bundle` with better diagnostics for different installation scenarios, bringing it in parity with what was already being done for host preflights. Support bundles now include different collectors based on whether it's an air gap installation and what proxy settings are configured, and adds additional network diagnostics collection.
