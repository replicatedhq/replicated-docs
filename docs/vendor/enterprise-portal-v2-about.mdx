...
- **Manage instances**: View all deployed instances and check for available updates. Helm and Embedded Cluster instances can expand into inline upgrade instructions. For KOTS and kURL, see [KOTS and kURL customers](#kots-and-kurl-customers).
...
- **Instance and update management**: The Instances & Updates page shows deployed instances and detects available updates. Helm and Embedded Cluster cards can expand into inline upgrade steps. For KOTS and kURL upgrades, see [KOTS and kURL customers](#kots-and-kurl-customers). The Download Portal has no instance concept.
...
The new Enterprise Portal supports KOTS and kURL customers, with some differences from Helm and Embedded Cluster customers. KOTS and kURL upgrades are performed in the Admin Console, which is the supported upgrade path for those installers. For details on what these customers see, including current limitations, see [KOTS and kURL instances](/vendor/enterprise-portal-v2-use#kots-and-kurl-instances) in _Log in to and Use the Enterprise Portal_.
