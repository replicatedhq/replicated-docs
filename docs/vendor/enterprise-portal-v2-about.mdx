...
- **Install and upgrade**: Step-by-step installation instructions for Helm, Embedded Cluster (Linux), KOTS, and kURL. The default content template includes dedicated KOTS and kURL install pages. For more information, see [KOTS and kURL customers](#kots-and-kurl-customers).
...
- **Manage instances**: View all deployed instances and check for available updates. Helm and Embedded Cluster instances expand into inline CLI instructions; KOTS and kURL instances expand into guided Admin Console application-upgrade steps. For KOTS and kURL, see [KOTS and kURL customers](#kots-and-kurl-customers).
...
- **Access security data**: Review CVE reports, filter to fixable vulnerabilities, view per-instance upgrade recommendations, and download SBOMs. Available for Helm and Embedded Cluster installs when Security Center is enabled
...
- **KOTS and kURL support**: Serves KOTS and kURL customers directly in the portal. They can view instances, browse versions, follow install instructions, and download air gap artifacts. The default content template includes dedicated KOTS and kURL install pages. These customers previously used the separate Download Portal. For more information, see [KOTS and kURL customers](#kots-and-kurl-customers).
...
Customers moving from the Download Portal gain the same content and automation capabilities. The Download Portal already shared KOTS and kURL artifacts. The Enterprise Portal also generates KOTS and kURL **install** CLI commands (KOTS air gap remains a documented procedure with placeholders). For upgrades, it provides target-aware Admin Console guidance and air gap application bundles rather than generating KOTS or kURL **upgrade** CLI commands in the portal UI. It also adds the following, which the Download Portal has no equivalent for:
...
- **Instance and update management**: The Instances & Updates page shows deployed instances and detects available updates. Helm and Embedded Cluster cards expand into inline CLI steps. KOTS and kURL cards expand into guided Admin Console application-upgrade steps, including the selected application bundle for entitled air gap customers. For details, see [KOTS and kURL customers](#kots-and-kurl-customers). The Download Portal has no instance concept.
...
When both portals are running for your team, **New Portal** / **Classic Portal** buttons appear in Vendor Portal. They are at the top of the main Enterprise Portal settings area. This is separate from any individual customer's tab. These buttons switch which version of Enterprise Portal settings you see and configure as a vendor. They have no effect on which portal any customer sees. To change a specific customer's portal, go to that customer's **Enterprise Portal access** tab and use their **Portal Version** setting instead.
...
The new Enterprise Portal supports KOTS and kURL customers, with some differences from Helm and Embedded Cluster customers. KOTS and kURL application upgrades are performed in Admin Console, and their instance cards provide target-aware guidance plus application bundle downloads for entitled air gap customers. For details on what these customers see, including current limitations, see [KOTS and kURL instances](/vendor/enterprise-portal-v2-use#kots-and-kurl-instances) in _Log in to and Use the Enterprise Portal_.
...
If you also want this customer to see Security Center, enable access for the customer or for all customers at the app level. Security Center access is separate from Portal Version. For a KOTS or kURL customer, Security Center shows an explanatory message instead of security data until they also have a Helm or Embedded Cluster instance. See [Security](/vendor/enterprise-portal-v2-use#security). For more information, see [Enable Security Center access](/vendor/security-center-enable-customer-access#enable-security-center-access) in _Enable Customer Access to Security Information_.
