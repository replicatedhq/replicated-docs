...
If you also want this customer to see Security Center, enable Security Center on the customer's **Enterprise Portal access** tab. This is a separate toggle from Portal Version and requires the Security Center feature flag to be enabled for your team. For a KOTS or kURL customer, Security Center shows an explanatory message instead of security data until they also have a Helm or Embedded Cluster instance. See [Security](/vendor/enterprise-portal-v2-use#security). For more information, see [Enterprise Portal (New) (Alpha)](/vendor/security-center-enable-customer-access#enterprise-portal-new-alpha) in _Enable Customer Access to Security Information_.
