...
- **Access security data**: Review CVE reports, identify the release where each CVE is fixed, filter to fixable vulnerabilities, view per-instance upgrade recommendations, and download SBOMs. Available for Helm and Embedded Cluster installs when Security Center is enabled
