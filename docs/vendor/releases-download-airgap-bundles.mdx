...
1. Build the `.airgap` bundle for the target release on the channel's **Release history** page. For more information, see [Build the Application Release Bundle in the Vendor Portal](#air-gap-bundle) below.

   <details>
      <summary>Why do I need to build the `.airgap` bundle if it is not required for Embedded Cluster air gap installations?</summary>
