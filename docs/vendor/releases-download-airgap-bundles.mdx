...
This topic describes how to build and download air gap bundles. It includes information about building and downloading bundles for application releases from the Replicated Vendor Portal. It also describes using the Replicated Download Portal to download air gap bundles.

## Overview

_Air gap bundles_ are required to install applications in air gap environments. Air gap bundles contain the images needed to install and run applications in an environment that has no outbound internet access.

### Required Air Gap Bundles

The air gap bundles required for installation vary depending on the installation type:

#### Replicated Embedded Cluster

Installations with Embedded Cluster require a single Embedded Cluster air gap bundle. The Embedded Cluster air gap bundle contains:
* All the assets contained in the `.airgap` bundle for the given application release (`airgap.yaml`, `app.tar.gz`, and an images directory)
...
:::note
While Embedded Cluster does not require users to supply a separate `.airgap` bundle for the specific application release, the `.airgap` bundle for the release must be built in the Vendor Portal before installation.
:::

#### Replicated KOTS in Existing Cluster

Installations with KOTS in an existing cluster require the following air gap bundles:
* **Application `.airgap` bundle**:  Assets required to install a specific release of the application (`airgap.yaml`, `app.tar.gz`, and an images directory)
* **KOTS Admin Console `kotsadm.tar.gz` bundle**: Contains the container images for the Replicated KOTS Admin Console. 

#### Replicated kURL

Installations with kURL require the following air gap bundles:
* **Application `.airgap` bundle**: Assets required to install a specific release of the application (`airgap.yaml`, `app.tar.gz`, and an images directory)
...
## Download the Embedded Cluster Bundle

For installations with Embedded Cluster, the Embedded Cluster air gap bundle is downloaded as part of the installation process.
...
1. After the `.airgap` bundle is built, click **Download Bundle**.
   ...
   :::note
   Downloading the `.airgap` bundle is not necessary for Embedded Cluster installations. The bundle is downloaded during installation using the Replicated app service.
   ::: 
...
Air gap installations with KOTS in existing clusters require the `kotsadm.tar.gz` air gap bundle, which includes the images for the KOTS admin console. The version of the `kotsadm.tar.gz` air gap bundle used must be compatible with the version of the application `.airgap` bundle.
