...
1. Build the `.airgap` bundle for the target release on the channel **Release history** page. For more information, see [Build the Application Release Bundle in the Vendor Portal](#air-gap-bundle) below.

   :::note
   Building the `.airgap` bundle is required because, if you attempt to download the Embedded Cluster air gap bundle without first bulding the `.airgap` bundle for the release, the API will return the Embedded Cluster tarball without the `.airgap` bundle. For more information, see [Limitations and Known Issues](/enterprise/installing-embedded-air-gap#limitations-and-known-issues) in _Air Gap Installation with Embedded Cluster_.
   ::: 
...
1. (Optional) After the `.airgap` bundle is built, click **Download Bundle**.
