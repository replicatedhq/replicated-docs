...
1. Run the `helm template` command with the `--set` flag to test that the preflights are working with the templating. For more information, see [Running Helm Preflights](preflight-running).
