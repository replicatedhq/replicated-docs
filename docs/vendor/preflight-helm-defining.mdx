...
When you run `helm template` command to produce a stream of stdout and pipe the result to `preflight -`, the `preflight` command filters the stream for and runs the following kinds:
...
-  `name`: Specify a `"{{ .Release.Namespace }}-NAME` to ensure that the specification is scoped to the release namespace. Replace `NAME` with the namespace where the release will be deployed.
