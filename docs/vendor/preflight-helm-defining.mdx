...
You must use conditional statements in the custom resource because it is not installed in the cluster. Instead, the custom resource is read using stdin.
...
1. Run the `helm template` command with the `--set` flag to test that the preflights are working with the templating. For example, `helm template mychart --set renderpreflights=true --values values.yaml | kubectl preflight -`.
