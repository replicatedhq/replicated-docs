...
For Helm, preflight checks can be run in two different ways, at different times. Replicated recommends that you use both of these methods to help ensure that your customer environment will support your application _before_ running the installation. Set up your preflight checks to run:

- Before running the `helm install` command, without hooks, to confirm the target cluster has the resources required for a successful install.
- During the installation, using `pre-install` and `pre-upgrade` hooks with `helm install`. The hooks run a Pod from the preflight image and supply the specification to that as a Secret or ConfigMap. The hooks run automatically immediately before installing the application, and prevent the installation if the preflight checks fail.
