...
- **Application slug:** The primary application slug is used with the Replicated CLI and with some of the KOTS CLI commands. You can click on the link below the slug to toggle between the application ID number and the slug name. The application ID and primary slug are unique identifiers that cannot be edited. You can reserve one additional slug as an alias. For more information, see [Reserve an Application Slug Alias](/vendor/application-slug-aliases).
