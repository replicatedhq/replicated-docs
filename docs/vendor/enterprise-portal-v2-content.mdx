...
- **Online CLI downloads**: The default page already mounts `<KotsDownloadAssets cliOnly={true} />` on the online/proxy branch. Lead that copy with customers who cannot run `curl https://kots.io/install`; the download is the install path for them, not a convenience on top of curl. Do not enable air gap on the license only to get the KOTS CLI. Do not move the air gap `<KotsDownloadAssets />` out of `<WhenNetwork mode="airgap">`. The default KOTS air gap procedure is unchanged.
