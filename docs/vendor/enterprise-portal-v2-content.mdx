...
**`<NetworkAvailability>`**: Selector for online, proxy, or air gap installation mode. The air gap option is shown only when the customer's license supports air gap. Proxy selection also shows a Proxy URL field (the URL is passed when generating instructions; it is not stored on the install-options record).

```markdown
<NetworkAvailability installType="linux" />
```

Props: `installType` (required: `"linux"`, `"helm"`, `"kots"`, or `"kurl"`). Pass `installType` so the air gap radio is gated on the correct entitlement. Without it, the air gap option is hidden.
...
**`<KotsInstallAssets />`**: Renders the full online or proxy KOTS existing-cluster installation command sequence. Generated commands can include `--license-file ./license.yaml`, `--app-version-label`, proxy flags (`--http-proxy` / `--https-proxy` / `--no-proxy`) when proxy is selected, and `--namespace` with the application slug when the slug is 63 characters or fewer. On non-stable channels the install positional is `appSlug/channelSlug`. Props: `stepNumber` (optional, starting step number).

**`<KurlInstallAssets />`**: Renders the full online or proxy kURL installation command sequence. Props: `stepNumber` (optional, starting step number).
...
For the KOTS and kURL step components, `step="install"` selects the corresponding `install_kots` or `install_kurl` step where that step exists.
...
KOTS and kURL instances already appear on the Instances & Updates page with no setup. Embedded Cluster and Helm instances can expand into generated CLI upgrade steps. KOTS and kURL cards do not expand: they show that updates are unavailable in-portal and direct the customer to upgrade in the Admin Console. The `<InstancesAndUpdates />` component in the default template renders this page.
...
- Online and proxy installs: generated commands from `<KotsInstallAssets />`, wrapped in `<WhenNetwork mode="online,proxy">`
...
**`<InstancesAndUpdates />`**: Renders the full instances and updates management page inline. Helm and Embedded Cluster instances can expand into inline upgrade CLI commands. KOTS and kURL instances show that in-portal updates are unavailable and direct the customer to the Admin Console; they do not expand into CLI steps. No props.
