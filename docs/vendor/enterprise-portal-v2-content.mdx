...
**`<KotsDownloadAssets />`**: Renders a KOTS downloads section with a version picker for the selected release. Available when the customer's license has the KOTS install type. The KOTS CLI and optional Preflight and Support Bundle CLIs appear even when air gap is disabled. The Admin Console bundle and the application air gap bundle appear only when air gap is enabled, and only when `cliOnly` is not set. This component does not render install commands. Do not unwrap the air gap instance to put CLIs on online: a customer who has air gap on the license but selected online would then see those bundles as required. Use a second mount with `cliOnly={true}` on the online/proxy branch instead (the default KOTS page does this). Props: `stepNumber`, `cliOnly` (optional).
...
- Online and proxy installs: generated commands from `<KotsInstallAssets />`, then optional workstation CLIs from `<KotsDownloadAssets cliOnly={true} />`, wrapped in `<WhenNetwork mode="online,proxy">`
...
- **Online CLI downloads**: The default page already mounts `<KotsDownloadAssets cliOnly={true} />` on the online/proxy branch. Do not enable air gap on the license only to get the KOTS CLI. Do not move the air gap `<KotsDownloadAssets />` out of `<WhenNetwork mode="airgap">`. The default KOTS air gap procedure is unchanged.
