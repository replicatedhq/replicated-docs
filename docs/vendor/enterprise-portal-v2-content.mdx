...
- **Online CLI downloads**: The default page already mounts `<KotsDownloadAssets cliOnly={true} />` on the online/proxy branch. Lead that copy with Preflight and Support Bundle (not curl-dependent). Then: if the customer cannot run `curl https://kots.io/install`, download the KOTS CLI, extract it, put `kubectl-kots` on PATH, and run the `kubectl kots install` line from step 2. Do not enable air gap on the license only to get the KOTS CLI. Do not move the air gap `<KotsDownloadAssets />` out of `<WhenNetwork mode="airgap">`. The default KOTS air gap procedure is unchanged.
