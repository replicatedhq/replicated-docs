...
The kURL guide separates an application update from kURL/Kubernetes cluster maintenance. The application panel does not render the kURL installer bundle and does not offer `install.sh` as a command. Vendors must document when cluster maintenance is required and provide a separately validated procedure.
