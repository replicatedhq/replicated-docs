...
**`<CVEReport />`**: Displays CVEs by severity, component, and container image for the selected release. For each CVE, the report identifies the earliest subsequent release where the CVE is no longer detected. The report also compares CVE counts with the latest release. Depending on your Security Center settings, the report shows only fixable CVEs or includes a toggle for customers to show all CVEs. No props.
