...
- **Online CLI downloads**: Do not enable air gap on the license only to get the KOTS CLI. If online customers should download CLIs from the portal (not only `curl https://kots.io/install`), place `<KotsDownloadAssets />` outside `<WhenNetwork mode="airgap">`. If the KOTS page is missing for those customers, drop `isAirgapSupported` from that page's `visible_when` and `toc.yaml` gate so it is `isKotsInstallEnabled` only.
