...
:::note
Application preflight specs use `troubleshoot.sh/v1beta2` for KOTS, kURL, and Embedded Cluster v2. Embedded Cluster v3 requires `troubleshoot.sh/v1beta3`, which is not supported by KOTS or Embedded Cluster v2. The requirement applies only to the Preflight custom resource. HostPreflight, support bundle, and Redactor specs still use `v1beta2` on all current installer products. See [Application preflight specs must use v1beta3](/embedded-cluster/v3/embedded-v3-migrate#preflight-specs-must-use-v1beta3).
:::
