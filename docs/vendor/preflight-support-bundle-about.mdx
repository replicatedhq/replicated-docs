...
During the redact phase, _redactors_ censor sensitive customer information from the data before analysis. For both preflight checks and support bundles, the following information is automatically redacted:
...
For support bundles, you can also add custom redactors to redact additional data, such as API keys or account numbers. Preflight checks apply the built-in redactors only. For more information, see [Redactor](/reference/custom-resource-redactor) and the [Redact](https://troubleshoot.sh/docs/redact/) section in the Troubleshoot documentation.
...
:::note
Preflight specs use `troubleshoot.sh/v1beta2` for KOTS, kURL, and Embedded Cluster v2. Embedded Cluster v3 requires `troubleshoot.sh/v1beta3`, which is not supported by KOTS or Embedded Cluster v2. Support bundle specs still use `v1beta2` on all current installer products. See [Preflight specs must use v1beta3](/embedded-cluster/v3/embedded-v3-migrate#preflight-specs-must-use-v1beta3).
:::
...
When an end customer generates a support bundle, the process uses the vendor-defined spec to collect data from the cluster, redact sensitive fields, and then perform an analysis on the data to provide remediation steps. The output is a compressed `tar.gz` file that can be shared with the vendor as part of a support escalation. For more information, see [How Customers Generate and Share Support Bundles](#generate-share) below.

:::note
No data is shared outside the cluster unless the `tar.gz` support bundle file is explicitly shared by the end customer.
:::

The following diagram demonstrates how end customers can generate support bundles in their installation environment, then upload the bundle to the Enterprise Portal and share it with the vendor. It also shows how, after a support bundle is uploaded and shared, vendors can inspect the bundle from the Vendor Portal **Support bundle analysis** page. 

![workflow of generating, uploading, and sharing a support bundle](/images/support-bundle-workflow.png)

[View a larger version of this image](/images/support-bundle-workflow.png)
...
You can also inspect a bundle locally with sbctl, which serves the bundle through a local Kubernetes API server so that you can explore it with `kubectl`. For more information, see [Inspect a support bundle locally with sbctl](/vendor/support-inspecting-support-bundles#sbctl).
