...
This quick start applies to applications that will be installed with Replicated Embedded Cluster. If your application will support Helm installations only, see [Helm-Only Quick Start](quick-start-helm).
...
* **KOTS**: Replicated KOTS is a kubectl plugin and in-cluster Admin Console that installs Kubernetes applications in customer-controlled environments.

* **Admin Console**: The Admin Console is the user interface for installing and managing applications with KOTS.

* **Embedded Cluster**: Replicated Embedded Cluster is a Kubernetes installer based on the open source Kubernetes distribution k0s. With Embedded Cluster, users install and manage both the cluster and the application together a single appliance on a VM or bare metal server. In this way, Kubernetes is _embedded_ alongside the application. Each version of Embedded Cluster includes a specific version of Replicated KOTS, which provides the Admin Console.

* **Replicated SDK**: The Replicated SDK is a Helm chart that can be installed as a small service alongside your application. It provides access to telemetry and custom metrics on instances of your application running in customer environments as well as an in-cluster API.

* **Vendor Portal**: The user interface for interacting with the Replicated Platform. 

* **Replicated CLI**: The CLI for interacting with the Replicated Platform.
...
1. Add the chart archive to a release in the Vendor Portal:
...
1. Install the application with Embedded Cluster:
    ...
    1. On the command line, SSH onto your VM.
...
1. Create a new release that adds preflight checks to the application:
   ...
   1. In the `gitea-preflights.yaml` file, add the following YAML to create a Kubernetes Secret with a preflight spec: 
...
1. On your VM, update the application instance to the new version that you just promoted:
   ...
   1. Follow the steps in the Embedded Cluster update wizard to deploy the new version.

## Next Steps

Congratulations! As part of this quick start, you:
* Added the Replicated SDK to a Helm chart
* Created a release with a Helm chart that supports installations with KOTS and Embedded Cluster
...
To get started, see [KOTS and Embedded Cluster Onboarding](distributing-workflow).
