...
You can customize the subject line, content, and styling of the emails that are sent to your customers. Email categories include Access and Authentication (invitations, magic links, verification codes) and Update Notifications (new release alerts).
...
To notify users when promoting a release, pass the `--notify-users` flag:
...
The `--notify-users` flag is also available as a `notify-users` parameter in the [Replicated GitHub Actions](https://github.com/replicatedhq/replicated-actions) for `create-release`.
