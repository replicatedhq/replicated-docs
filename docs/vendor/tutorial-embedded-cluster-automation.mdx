...
# SlackerNews Automated (Headless) Installation
...
1. View the contents of the chart.   

1. Package the Helm chart to a `.tgz` chart archive:
...
1. Download the release manifests:
    ...
    ```bash
   curl -O --create-dirs --output-dir tutorial-headless-install https://docs.replicated.com/slackernews-1.0.1-manifests.tar.gz
   ```

1. Untar:

    ```bash
    tar -xzf tutorial-headless-install/slackernews-1.0.1-manifests.tar.gz -C tutorial-headless-install/ && rm tutorial-headless-install/slackernews-1.0.1-manifests.tar.gz
    ```
...
1. Create the ConfigValues file. This file is passed to the installation using the `--application-config` flag.
