...
1. Copy the URL for the DNS record with a target port of 443 (the hostname for accessing SlackerNews) and save it somewhere on your local machine. You will provide this hostname in the ConfigValues file that you created in the next step.
...
1. In `slackernews-configvalues.yaml`, add the following YAML:    
     ...
     values:
       slackernews_domain:
         value: YOUR_DOMAIN
     ```
     Where `YOUR_DOMAIN` is the domain to use for SlackerNews. For Compatibility Matrix VMs, this is the hostname for the DNS record with port 443.
...
1. (Optional) Go to the domain for the Admin Console and log in using the password that you set with the `--admin-console-password` flag during installation.

    The Admin Console domain is the hostname for the DNS record with port 30000 that you added to the VM. You can find this hostname by going to **Compatibility Matrix > Edit VM** in the Vendor Portal.
