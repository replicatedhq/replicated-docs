...
1. On your VM, open a shell so that you can interact with the cluster with the kubectl command line:

   ```
   sudo ./APP_SLUG shell
   ```

1. Watch for the resources become ready:

   ```bash
   kubectl get all --namespace slackernews --watch
   ```

1. After the deployment is running, go to the domain for SlackerNews to open the application and confirm that the installation was successful.

1. (Optional) Go to the domain for the Admin Console and log in.