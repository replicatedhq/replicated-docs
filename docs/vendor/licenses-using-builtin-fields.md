...
| `isSemverRequired` | If set to `true`, this field requires that the admin console orders releases according to Semantic Versioning. For more information about enabling Semantic Versioning on a channel, see [Enabling Semantic Versioning](releases-semantic-versioning). |
| `isSnapshotSupported` | If a license supports the snapshots backup and restore feature, this field is set to `true`. If a license does not support snapshots, this field is either missing or `false`. |
