...
The new Enterprise Portal uses a GitHub content repo and `theme.yaml` for customization instead of Vendor Portal UI forms. See [Customize Portal Content (New)](/vendor/enterprise-portal-v2-content) and [Customize Portal Branding (New)](/vendor/enterprise-portal-v2-branding).
:::
...
This section describes options for customizing the emails sent to customers through the Enteprise Portal. For information about how to view the delivery status of all emails sent to customers, see [View Email History and Delivery Status](/vendor/enterprise-portal-invite#email-history) in _Manage Enterprise Portal Customer Access_.

### Configure email sender

Adding a sender address helps ensure that your emails are delivered and are not marked as spam.
...
1. Follow the steps in the wizard to verify the domain by adding the DKIM and Return-Path to your domain registrar.
...
### Exclude certain images from Helm CLI install instructions {#installer-only}
...
<table>
  ...
  <tr>
    <td><code>{`{installation.customerEmail}`}</code></td>
    <td>Email address associated with the customer installation. Used for registry authentication as the username in Helm CLI installation instructions.</td>
    <td>`customer@example.com`</td>
  </tr>
  <tr>
    <td><code>{`{installation.serviceAccountToken}`}</code></td>
    <td>Authentication token for the service account. Used for registry authentication as the password in Helm CLI installation instructions.</td>
