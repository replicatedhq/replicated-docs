...
```json
{
  "scans": [
    {
      "input": "example.com/app/api:1.2.3",
...
Scans run asynchronously and continuously, so a scan might not be complete when you request the results. While a scan is in progress, its `scan_status` is a non-terminal value such as `pending` or `in_progress`, and its `result` is `null`. A completed scan has a `scan_status` of `succeeded`, and a scan that could not complete has a `scan_status` of `failed`. Poll the endpoint until every scan reaches a terminal status before you evaluate the results.
...
Each image also includes a `source_type` of `vendor` or `replicated`. Use this field to distinguish your application images from Replicated Platform images.

The response includes a top-level `summary` object with the following fields:

* `total_images`: Total images in the response, including images with incomplete scans
* `vendor_image_count`: Images classified as application images
* `replicated_image_count`: Images classified as Replicated Platform images
* `vendor_vuln_counts`: Vulnerability counts across application images
* `replicated_vuln_counts`: Vulnerability counts across Replicated Platform images
