...
The purpose of the KOTS HelmChart custom resource [`builder`](/reference/custom-resource-helmchart-v2#builder) key is to set Helm values for rendering chart templates so that the output includes any required images for the `.airgap` bundle.

You should configure the `builders` key if you need to change any default values in your Helm chart so that the `.airgap` bundle for the release includes all images needed to successfully deploy the chart. For example, if there is a `required` field in your Helm chart that must be set for the chart to render or if you need to change which image is used for air gap installations.
