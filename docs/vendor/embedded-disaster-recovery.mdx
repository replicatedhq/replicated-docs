...
The Embedded Cluster disaster recovery feature allows you to support backup and restore for your customers. Your customers can configure and take backups from the Admin Console and restore from the command line.
...
1. In a new release containing your application files, add a Velero Backup resource. In the Backup resource, indicate the labels of the resources that you want to include in the backup.

      For more information about the available fields, see [Backup API Type](https://velero.io/docs/latest/api-types/backup/) in the Velero documentation.

1. In the same release, add a Velero Restore resource.

      For more information about the available fields, see [Restore API Type](https://velero.io/docs/latest/api-types/restore/) in the Velero documentation.
