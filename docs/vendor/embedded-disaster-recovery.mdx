...
1. Run the restore command:
     ...
     Note the following requirements for the `restore` command:
       ...
       * If the `--local-artifact-mirror` flag was used during install to relocate the LAM port, specify a new port with the `restore` command. For example, `restore --local-artifact-mirror-port=50000`. Otherwise, the port from the backup will be used. Note that the `--admin-console-port` flag is not supported during restores. For more information, see [Relocate Admin Console and LAM Ports](/enterprise/installing-embedded#relocate-admin-console-and-lam-ports).
