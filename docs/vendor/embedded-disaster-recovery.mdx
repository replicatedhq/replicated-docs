...
:::important
Embedded Cluster disaster recovery in an Alpha feature. This feature is subject to change, including breaking changes. To get access to this feature, reach out to Alex Parker at [alexp@replicated.com](mailto:alexp@replicated.com).
:::
...
1. (Optional) In addition to the previous steps, you can use Velero functionality like [backup](https://velero.io/docs/main/backup-hooks/) and [restore](https://velero.io/docs/main/restore-hooks/) hooks to customize your backup and restore process as needed.
     ...
     podAnnotations:
       ...
       # wait's for the pod to be ready before running the post-restore hook
...
## Configure Backup Storage in the Admin Console

You customers need to configure backup storage from the Admin Console before they can take backups and perform restores. 

To configure backup storage for disaster recovery in the Admin Console:
...
1. For the desired S3-compatible backup storage location, enter the bucket, prefix (optional), access key ID, access key secret, endpoint, and region.

    ![backup storage settings](/images/dr-backup-storage-settings.png)

1. Click **Update storage settings**.

## Take Backups in the Admin Console

After configuring backup storage, you can take backups in the Admin Console and also schedule automatic backups.

To take backups in the Admin Console:

1. In the Admin Console, go to the **Disaster Recovery** tab and click the **Backups in the Disaster Recovery** submenu.

     Backups can be taken from this screen.
...
1. Download the Embedded Cluster binary for the version of the application that was included in the backup. See [Download the Embedded Cluster Binary] for more information.
