...
Additionally, you must instruct your end users to use the runtime flags for installing the admin console without MinIO for Kubernetes Installer and existing clusters when using these air gap packages. For more information about setting the flags for installing without MinIO, see [Install the Admin Console Without MinIO](https://docs.replicated.com/enterprise/installing-stateful-component-requirements#install-the-admin-console-without-minio) in _Requirements for Admin Console State_.
