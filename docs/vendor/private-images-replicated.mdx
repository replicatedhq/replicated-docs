...
There is a known issue with the Replicated registry that can cause a timeout error when attempting to push images. This can result in an error message similar to the following: `received unexpected HTTP status: 524`.
