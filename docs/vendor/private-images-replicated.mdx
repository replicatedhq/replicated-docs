...
* The Replicated registry cannot process image layers larger than 2GB. Build your images so that the image layers are less than or equal to 2GB before pushing to the Replicated registry. This limitation does not limit the overall size of images that can be pushed to the registry. 
