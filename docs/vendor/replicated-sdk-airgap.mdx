...
When the SDK is installed with the Helm CLI in air gap environments, air gap mode can be enabled by passing `--set replicated.isAirgap=true` with the Helm CLI installation command.
