...
1. Create a Secret (`kind: Secret`) containing the specification. Alternatively, you can use a ConfigMap (`kind: configMap`) if the specification will not contain private information.
...
1. When using Secrets, you can also add templating for the specification in the `values.yaml` file.