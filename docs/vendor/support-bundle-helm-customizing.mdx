...
If you do not want to use Secrets, you can create a SupportBundle custom resource (`kind: SupportBundle`) instead. You share this specification through a URL, at a registry link, or manually as a file. Helm templates are supported when the specification is distributed using an OCI registry.

Create a SupportBundle custom resource manifest file (`kind: SupportBundle`) using the following basic support bundle template. For more information about this custom resource, see [Preflight and Support Bundle](/reference/custom-resource-preflight).
