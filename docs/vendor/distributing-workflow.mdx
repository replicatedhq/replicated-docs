...
1. Install the release to test your changes. See [Performing Udpates in Embedded Clusters](/enterprise/updating-embedded) and [Performing Updates in Existing Clusters](/enterprise/updating-app-manager).
...
### Task 7: Add Custom Domains

<CustomDomains/>

### Task 8: Add Custom License Entitlements
...
Congratulations! After completing the main onboarding tasks, Replicated recommends that you also complete the following additional tasks to integrate other Replicated features with your application. 
...
Before you begin, Replicated recommends that you complete the [Deploy a Helm Chart with KOTS and the Helm CLI](tutorial-kots-helm-setup) tutorial to learn how to install a single release with both KOTS and the Helm CLI in an existing cluster.
