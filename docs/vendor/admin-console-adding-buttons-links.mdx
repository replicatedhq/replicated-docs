...
### Rewrite URLs Using KOTS

When providing a link to your application, there are some cases where it might not be possible to construct the URL ahead of installation. For example:
...
In these cases, it is possible to use KOTS to automatically rewrite the URL at the time of installation, including using the hostname in the browser (where the Admin Console is being accessed) and appending a local port.
