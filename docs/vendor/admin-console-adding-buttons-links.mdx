...
The type of service that you can add to the `ports` key depends on the application installation method:
* For existing cluster installations, you can add ClusterIP, NodePort, or LoadBalancer services to the `ports` key
* For Embedded Cluster or kURL installations, you can add NodePort services to the `ports` key

For more information about Kubernetes service types, see [Service](https://kubernetes.io/docs/concepts/services-networking/service/) in the Kubernetes documentation.

### How To

To add a link to an application service using the `ports` key:
