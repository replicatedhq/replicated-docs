...
For examples of how to configure the KOTS `ports` key to link to port-forwarded services in existing cluster installations or how to link to NodePort services in installations on VMs or bare metal servers, see [Examples](admin-console-port-forward#examples) in _Port Forwarding Services with KOTS_.

:::note
Using KOTS to automatically rewrite URLs is particularly useful when you are developing and testing KOTS releases for your application, as it provides an easier way to access the application after installation. However, for production releases, it is more common to have the user provide an IP address or hostname during installation as described in [Create URLs with User-Supplied Values Using KOTS Template Functions](#url-template) above.
:::