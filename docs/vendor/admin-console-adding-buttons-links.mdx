...
KOTS includes logic that can rewrite the URL provided in the Kubernetes SIG Application custom resource. 

This allows you to add a link to an application service that can be accessed after the application is installed, before ingress or any firewalls are set up.

configure the Kubernetes SIG Application and KOTS Application custom resources to 
...
that is either port-forwarded with KOTS (for existing cluster installations) or is a NodePort type service (for embedded installations with kURL or Embedded Cluster).