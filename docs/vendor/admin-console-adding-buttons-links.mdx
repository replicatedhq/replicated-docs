...
This seciton describes how to use KOTS to expose and add links to services from the Admin Console dashboard.

### About Exposing Services with KOTS

The KOTS Application custom resource [`ports`](/reference/custom-resource-application#ports) key allows you to expose services to users after an application is installed. This can make it easier for users to access your application before cluster ingress is configured.

For existing cluster installations, configuring the `ports` key adds the service to the KOTS port forward tunnel. See [Port Forwarding Services with KOTS](/vendor/admin-console-port-forward). For installations on VMs or bare metal servers with Replicated kURL or Replicated Embedded Cluster, configuring the `ports` key allows you to expose additional ports on the node to make NodePort services accessible to users. See [Exposing Services Using NodePorts](/vendor/kurl-nodeport-services).

### About Linking to Services

You can add a link to an application service on the Admin Console dashboard.

To do so, you configure both the `ports.applicationURL` field in the KOTS Application custom resource and the `spec.descriptor.links` field in the Kubernetes SIG Application custom resource. During deployment, KOTS rewrites the URL supplied in the Kubernetes SIG Application custom resource with the hostname in the browser (the hostname where the Admin Console is accessed). As part of this process, KOTS also appends the service port to the URL so that users can access the service from their browser after the application is installed.
...
To add a link to a service for installations with Replicated kURL or Replicated Embedded Cluster, first create a NodePort type service.
