...
You can configure the KOTS Application custom resource [`ports`](/reference/custom-resource-application#ports) key to add links to services on the Admin Console dashboard. This can make it easier for the person or process installing your application to easily verify that the application is running, since it can be difficult to provide a consistent application URL at installation. For example, the cluster operator might be required to configure ingress or create firewall rules before they can open the application. 

To add a link to a service using the KOTS `ports` key, add the _same_ URL in the KOTS Application custom resource `ports.applicationURL` and Kubernetes SIG Application custom resource `spec.descriptor.links.url` fields. When the URLs in these fields match, KOTS automatically rewrites the URL during deployment with the hostname in the browser (the hostname where the Admin Console is accessed) and appends a specified port.
...
* For Replicated Embedded Cluster or Replicated kURL installations, see [Exposing Services Using NodePorts](/vendor/kurl-nodeport-services)