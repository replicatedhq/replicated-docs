...
The KOTS Application custom resource [`ports`](/reference/custom-resource-application#ports) key allows you to expose services to users after an application is installed. This can make it easier for users to access your application before cluster ingress is configured. For more information about using the `ports` key to port forward services in existing cluster installations, see [Port Forwarding Services with KOTS](/vendor/admin-console-port-forward). For more information about using the `ports` key to expose NodePort service in installations with Replicated kURL or Replicated Embedded Cluster, see [Exposing Services Using NodePorts](/vendor/kurl-nodeport-services).
