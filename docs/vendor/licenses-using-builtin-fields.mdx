...
| `isGitOpsSupported` | <GitOpsNotRecommended/> (KOTS Only) If a license supports gitops-enablement in the admin console, this field is set to `true`. If GitOps is not supported, this field is either `false` or missing. See [Pushing Updates to a GitOps Workflow](/enterprise/gitops-workflow).|
