...
The `values` field in the HelmChart custom resource lets you edit the Helm chart `values.yaml` file. A common use case for the HelmChart `values` field is to use Replicated template functions to map user-provided values from the Replicated admin console Configuration screen to the Helm chart `values.yaml` field.
