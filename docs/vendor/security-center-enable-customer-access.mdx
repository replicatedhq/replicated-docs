...
Customers can select a release and review CVEs by severity, affected component, and container image. The CVE report identifies the earliest subsequent release on the customer's channel where each CVE is no longer detected. Customers can also compare the selected release with the latest release, follow links to upgrade affected instances, and download an SBOM.
