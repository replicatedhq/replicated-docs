...
1. Copy and paste the following YAML into the file under the `example_default_value` field to create a new text field on the admin console configuration page:
...
1. Save the changes to both YAML files, then run the following command to verify that there are no errors in the YAML:
...
1. Run the following command to verify that the release was promoted to the Unstable channel:
