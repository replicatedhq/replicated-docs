...
The values in the `builder` key map to values in the corresponding Helm chart `values.yaml`. In the example below, `builder.postres.enabled` in the HelmChart custom resource maps to `postres.enabled` in the Helm values:
...
The `builder` key contains static (hardcoded) values that override the corresponding Helm values when KOTS templates the Helm chart to build the `.airgap` bundle for the release. This means that you can set values in the `builder` key that are specific to air gap installations.

For examples of common use cases for the `builder` key, see the sections below.

### Example: Set the Image Registry for Air Gap Installations

For air gap installations, it might be required to change the default image registry where certain images are pulled. You can use the `builder` key to do this by hardcoding the target values for the image registry, repository, and tag for air gap installations, as shown in the example below:
...
Many applications include resources that are included or excluded from a deployment based on a given condition. For example, enterprise users might have the option of deploying an embedded database with the application or bringing their own database. To support this use case for air gap installations, the optional resource must be included in the air gap bundle for the release.
