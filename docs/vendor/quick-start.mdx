...
1. Get the sample SlackerNews Helm chart:
   ...
   1. Change to the `slackernews` chart directory:
      ...
      ```
      The following shows the files that you will see in the `slackernews` directory:
      ```bash
...
1. If you brought your own VM, SSH onto your VM. Otherwise, follow these steps to create and SSH onto a VM with Compatibility Matrix:
   ...
   1. For **OS distribution > Distribution**, select Ubuntu. **OS distribution > Version**, select 24.04.
   
   1. (Optional) For **Set TTL**, select **2 hours** or longer. By default, the TTL is one hour. After the TTL expires, Compatibility Matrix automatically deletes the VM. If you want more time to complete this quick start tutorial, you can extend the TTL to two or more hours.
   
   1. On the **VM Config** tab, for **Instance type** select `r1.small`. This instance type meets the requirements for Embedded Cluster and SlackerNews.
...
1. Install the application with Embedded Cluster:
   ...
   1. When prompted, enter a password for accessing the Admin Console. You will use this password to log in to the Admin Console in a later step.
   ...
   1. Do one of the following to access the Admin Console:
       * If you created the VM with Compatibility Matrix, go to the **Compatibility Matrix** page in the Vendor Portal and copy the URL provided for the DNS record that you added in an earlier step. This URL will provide a tunnel so that you can access the Admin Console from your local machine.
   ...
   1. On the application config page, leave the default application configuration settings. Click **Continue**.
