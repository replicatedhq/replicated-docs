...
* **Option 1: Use Compatibility Matrix.** To use Compatibility Matrix to create a VM, do the following before proceeding:
   ...
   * If you don't have one already, add an SSH key to your GitHub account. This will be used when you SSH onto the VM during the tutorial.
   
   * In your Vendor Portal [**Account Settings**](https://vendor.replicated.com/account-settings), add your GitHub username.

   After you complete these prerequisites, continue to the [Quick Start](#quick-start). You will create the VM with Compatibility Matrix as part of the tutorial.

* **Option 2: Bring your own VM.** Your VM must meet these requirements for Embedded Cluster:
...
1. Add the `slackernews-0.4.14.tgz` chart archive to a release in the Replicated Platform:
   ...
   1. In each of the files that you created, paste the corresponding YAML provided in the tabs below:
      ...
      <h5>Description</h5>
      <p>The Config custom resource defines the application configuration fields that are exposed to the user in the Admin Console. </p>
      <h5>YAML</h5>
      <ConfigCr/>
      </TabItem>
      </Tabs>
   ...
   1. Lint the YAML files:
      ...
      **Example output:**
      ...
      ```
      :::note
      You can ignore these warning messages for the purpose of this quick start.
      :::

   1. From the `manifests` directory, create the release and promote it to the Unstable channel:
...
1. Create a test customer so that you can install the release with Embedded Cluster:
   ...
   1. For **Install types**, enable **Embedded Cluster (current generation product)** and disable the other options.
...
1. If you brought your own VM, SSH onto your VM. Otherwise, follow these steps to create and SSH onto a VM with Compatibility Matrix:
   ...
   1. Run the following command to create a VM:
      ...
      ```
      In about one minute, the status of the VM changes from `queued` to `running`.

   1. Check the status of the VM to see when it is `running`:

      ```bash
      replicated vm ls
      ```

   1. When the VM is `running`, copy the VM ID from the output of the `replicated vm ls` command.
...
1. Install the application with Embedded Cluster:
    ...
    1. On the page for the customer that you created, click **Embedded Cluster install instructions**.
    ...
    1. On the VM, run the commands in the **Embedded cluster install instructions** dialog to download the latest release, extract the installation assets, and install.
   ...
   1. When prompted, set a password for accessing the Admin Console. Save the password in a secure location. You will use this password to log in to the Admin Console in a later step.
   ...
   1. Do one of the following to access the Admin Console:
       ...
       * If you created the VM with Compatibility Matrix, create DNS records so that you can access both the Admin Console and SlackerNews from a browser on your local machine:
         1. In the Vendor Portal, go to **Compatibility Matrix**.
         1. Open the dot menu for your VM and click **Edit**.
         ...
         1. Under **Ingress & Ports**, for **Add DNS record**, click **Add** to create a DNS record using the default settings. This will provide a hostname where you can access the Admin Console on port 30000.

         1. Add another DNS record with a **Target Port** of **443**. This will provide a hostname where you can access SlackerNews on port 443.

             The following shows an example of a VM with both of the necessary DNS records:
         ...
         1. Copy the URL provided for the DNS record.
   ...
   1. On the **Configure SlackerNews** screen, complete the following fields:
       ...
       * For **Hostname**:
          * If you created the VM with Compatibility Matrix, enter the hostname for the DNS record that you created with the target port of 443. For example, `pensive-keldysh.ingress.replicatedcluster.com`.
          * If you are using your own VM, provide the IP address where you are currently accessing the Admin Console. For example, `34.10.177.196`.
       ...
       * For **Postgresql**, ensure that **Deploy Postgresql Database** is enabled to deploy the built-in Postgresql database.
       ...
       Leave the other fields on the **Configure SlackerNews** screen empty.
   ...
   1. Click **Continue**.
       ...
       ![Slackernews admin console dashboard](/images/quick-start-slackernews-dashboard.png)

       [View a larger version of this image](/images/quick-start-slackernews-dashboard.png)
   ...
   1. After a few minutes when the application status is Ready, click **Open SlackerNews** to view SlackerNews in a browser.
      ...
      <img alt="Slackernews landing page" src="/images/slackernews-landing-page.png" width="800px"/>
...
1. Create a new release that adds preflight checks to the application:
   ...
   1. From the `manifests` directory, lint the release manifests:
      ...
      :::note
      If you see a `helm-archive-missing` error, run `rm -rf .history` and then try linting the files again.
      :::
...
1. Clean up the installation:
    ...
    * If you brought your own VM, reset and reboot the VM to remove the installation:
