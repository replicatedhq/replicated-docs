...
1. Add the chart archive to a release:
   ...
   1. In the `slackernews` directory, create a subdirectory named `manifests`:
   ...
   1. Move the Helm chart archive that you created to `manifests`:
      ...
      ```
      mv slackernews-0.4.14.tgz manifests
      ```
...
1. Create a new release that adds preflight checks to the application:
   ...
   1. In your local filesystem, go to the `slackernews` directory.

   1. Create a `slackernews-preflights.yaml` file in the `templates` directory:
