...
1. If you brought your own VM, SSH onto your VM. Otherwise, follow these steps to create and SSH onto a VM with Compatibility Matrix:
   ...
   1. Run the following command to create a VM:
      ...
      ```
   1. Run the following command to check the status of the VM:

      ```bash
      replicated vm ls
      ```

   1. When the VM status is `running`, copy the VM ID from the output of the `replicated vm ls` command.
   
   1. Run the following command to use the Compatibility Matrix Forwarder to SSH into the VM:
...
1. Install the application with Embedded Cluster:
   ...
   1. On the application config page, complete the following fields:
       ...
       * For Service type, select **NodePort** and leave the port field blank to use the default port of 443. 
       * Ensure that **Deploy Postgres** is enabled to deploy the built-in postgres database.
