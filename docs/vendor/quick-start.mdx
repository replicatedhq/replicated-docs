...
1. Add the `slackernews-0.4.14.tgz` chart archive to a release:
...
1. Install the application with Embedded Cluster:
   ...
   1. Do one of the following to access the Admin Console:
       * If you created the VM with Compatibility Matrix, do the following to create a tunnel so that you can access the Admin Console from your local machine:
   ...
   1. On the **Configure SlackerNews** screen, complete the following fields:
       ...
       * For **Hostname**:
          ...
          * If you are using your own VM, provide the IP address where you are currently accessing the Admin Console with `:443` appended using the following format: `YOUR_IP_ADDRESS:443`. For example, `34.10.177.196:443`.
