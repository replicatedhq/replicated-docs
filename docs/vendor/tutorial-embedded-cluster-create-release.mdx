...
Next, add the Helm chart archive to a new release for the application in the Replicated vendor platform. The purpose of this step is to configure a release that supports installation with Replicated embedded cluster.
...
1. In each of the files that you created, paste the corresponding YAML provided in the tabs below:
   ...
   <h5>Description</h5>
    <p>To install your application with embedded cluster, an embedded cluster Config must be present in the release. The embedded cluster Config lets you define several characteristics about the cluster that will be created.</p>
    <h5>YAML</h5>
    <EcCr/>
   </TabItem>
   </Tabs>
...
Create a customer with the embedded cluster entitlement so that you can install the release in your cluster using embedded cluster. See [Step 5: Create an Embedded Cluster-Enabled Customer](tutorial-embedded-cluster-create-customer).
