...
1. Under **Ingress & Ports**, for **Add DNS record**, click **Add** to create a DNS record with the default **Target Port** of **30000**.
...
1. When prompted, provide the passphrase for the SSH key in your linked GitHub account.
...
1. Return the SCP endpoint for the VM:
...
1. (Optional) In a browser, go to the Admin Console.

   :::note
   You generated a URL for the Admin Console as part of [Create a VM with CMX](#create-a-vm-with-cmx). You can find the URL for the Admin Console by going to **Compatibility Matrix > Edit VM > Ingress & Ports** in the Vendor Portal and copying the URL for the DNS record with a target port of 30000.
   :::
