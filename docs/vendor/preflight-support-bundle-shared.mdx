...
1. Finally, include the preflight and support bundle specs in Kubernetes Secrets. This allows the specs to be discovered when users run preflight checks and generate support bundles.

    **Example:**

    In the example below, the `"troubleshoot.preflights"` and `"troubleshoot.supportBundle"` templates are included in two different Secrets as string data.
