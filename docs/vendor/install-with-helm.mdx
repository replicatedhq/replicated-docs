...
1. In the **Helm install instructions** dialog, run the first command to log in:
     ...
     * `LICENSE_ID` is the ID of the customer's license
     ...
     To avoid the warning, click **(show advanced)** to use a command that prompts for the password. The hostname shown in the dialog can be different for older accounts; use the displayed command.
     :::
