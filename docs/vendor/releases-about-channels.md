...
The following describes each of the fields in the **Channel Settings** dialog shown above:
...
* **Custom domain overrides**: Select the customer-facing domains that releases promoted to this channel use for the Replicated private registry, proxy service, or download portal endpoints. If a default custom domain exists for any of these endpoints, choosing a different domain in the channel settings overrides the default. Alternatively, select **No override** to use either the default custom domain or the default Replicated domain if no default custom domain exists. If no custom domains are configured for an endpoint, the drop-down for the endpoint is disabled.
