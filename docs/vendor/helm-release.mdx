...
1. In the **Select Helm Install Method** dialog that displays, select the method that the app manager uses to install the Helm chart. Replicated recommends that you select the Native Helm installation method.
   ...
   After you select the installation method, the vendor portal automatically creates a HelmChart custom resource manifest file for the Helm chart in the release.
...
1. Edit the files in the release as needed. You can also drag and drop additional application manifest files or click the plus icon to add new, untitled files.
   ...
   For information about how to edit the HelmChart custom resource manifest file, see [HelmChart](/reference/custom-resource-helmchart) in _Reference_.
