...
`useHelmInstall` is a chart-level flag that can only be set to `true` for new charts. Modifying existing charts in existing applications is not supported because charts installed with Replicated Helm cannot be migrated to Native Helm.
