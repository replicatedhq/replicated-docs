...
1. (Optional) Initialize the project as a Git repository.
...
1. Create a new release or update an existing release. You can continue to update and lint an existing release until you are ready to promote it. Releases cannot be edited after they are promoted.
      ...
      * Create and promote a new release with the default settings, which includes the Unstable channel:
      ...
      * Create and promote a new release with custom settings, such as a specific channel:
   ...
   For more information, see [release create](../reference/replicated-cli-release-create), [release promote](/reference/replicated-cli-release-promote), and [release update](../reference/replicated-cli-release-update) in the replicated CLI documentation.

1. After you promote the release, verify that it was promoted to the channel:
...
- Learn about the supported helm CLI installation options. See [Supporting helm CLI installations (Beta)] for your customers.
- Learn about including the Replicated admin console to your Helm release. See [Delivering the Admin Console with your Application](helm-install#deliver-admin-console).
- Test the release in your development environment.