...
This topic describes how to add a Helm chart to a release in the Replicated vendor portal and using the replicated CLI. Adding a Helm chart to a release is useful if you want to distribute a Kubernetes application with Replicated that is already packaged using Helm.
