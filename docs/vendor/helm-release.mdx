...
## Add the Helm Chart to a Release      
 ...
 You can create and promote a release with the Helm chart package using one of the following methods:
...
You must create the Helm chart `.tgz` package before you can create a release with a Helm chart. For more information, see [Creating a Helm Chart Package](helm-release-creating-package).
...
1. Drag and drop the Helm chart `.tgz` package that you created as part of [Create a Helm Chart Package](helm-release-create-package) into the file directory in the YAML editor. 
