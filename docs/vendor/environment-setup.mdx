...
sbctl is an open source project that is maintained by Replicated. It is not part of the Replicated platform, and you install and run it yourself. For more information, see the [sbctl](https://github.com/replicatedhq/sbctl) repository in GitHub.

For more information about using sbctl, see [Inspect a support bundle locally with sbctl](/vendor/support-inspecting-support-bundles#sbctl).

On macOS, install sbctl with Homebrew:
...
Or install it directly without adding the tap first:

```bash
brew install replicatedhq/replicated/sbctl
```

To install sbctl manually, use the instructions for your platform:
