...
:::important
This example templates a database password into the collector `uri`, which does not keep the credential secret. The template resolves to the live credential when the spec is rendered and deployed to the cluster, where a support bundle can collect it in plain text. Replicated recommends that you use Helm template functions only for non-sensitive values, such as the host, port, and database name. For more information, see [Pass a credential to the pod](/vendor/support-bundle-examples#run-pod-credentials) in _Example support bundle specs_.
:::
