...
When you run preflight checks with KOTS, you add the Preflight custom resource (`kind: Preflight`) to your release.

## Create a Preflight Custom Resource

Create a Preflight custom resource for KOTS. The following example shows a basic Preflight custom resource manifest file (`kind: Preflight`) in your release.
