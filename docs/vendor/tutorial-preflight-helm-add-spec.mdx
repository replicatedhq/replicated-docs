...
Next, add the preflight check specification to the Gitea chart as a Kubernetes Secret. Including the preflight spec in a Secret in the templates of a Helm chart allows you to deliver the preflight checks are part of your application.

1. Move the `gitea-preflights.yaml` file to the Gitea `templates` subdirectory:

   ```
   mv gitea-preflights.yaml gitea/templates
   ```

1. In the `gitea-preflights.yaml` file, add the following YAML to create a Kubernetes Secret with the required label `troubleshoot.sh/kind: preflight`. The preflight spec that you created is added to this Secret under `stringData`: 
