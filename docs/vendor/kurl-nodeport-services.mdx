...
After installation, you can provide access to NodePort services by adding a link to the service on the Admin Console dashboard. Additional configuration is required to add a link to the dashboard.
...
### Link to a URL Using User-Supplied Values {#user-supplied}

You can add a Kubernetes SIG Application custom resource in your release to provide a link to the NodePort service.  

If your users will supply an IP address or hostname during installation (for example, if users set up an ingress controller or a load balancer), you can use [KOTS template functions](/reference/template-funcions-about) to construct the URL for the link using user-supplied values.
...
```
In the example above, the [ConfigOption](/reference/template-functions-config-context#configoption) template function renders a hostname and node port provided by the user on the KOTS Config screen during installation to provide access to the NodePort service.

For more information, see [Adding Links to the Dashboard](admin-console-adding-buttons-links).

### Rewrite the URL Using KOTS

There are some cases where it might not be possible to create a URL with user-supplied values, as described in [Link to a URL Using User-Supplied Values](#user-supplied) above. For example:
* You do not want to require users to set up ingress or a load balancer and/or provide an IP address or hostname on the KOTS Config screen during initial installation
* The hostname or IP address cannot be determined

In these cases, you can use KOTS to automatically rewrite the URL at the time of installation to:
* Use the hostname in the browser where the Admin Console is accessed
* Append a local port to the URL

To use KOTS to rewrite the URL in the Kubernetes SIG Application custom resource, you can configure the KOTS Application custom resource `ports` key.
