...
For installations on VMs or bare metal servers where your application must be accessible from the user's local machine rather than from inside the cluster, you can expose application services as NodePorts to provide quick access to the application after installation.
...
After configuring a NodePort service for your application, you can add a link to the service on the Admin Console dashboard where it can be accessed by users after the application is installed. For more information, see [Add a Link to a NodePort Service from the Admin Console](#add-link) below.  
...
## About Adding Links to NodePort Services on the Admin Console Dashboard {#add-link}

This section describes providing access to NodePort services after installation by adding a link to the service on the Admin Console dashboard.

### Add a Link in the Kubernetes SIG Application Custom Resource

After installation, you can provide a link to a NodePort service on the Admin Console dashboard by configuring the `links` array in the Kubernetes SIG Application custom resource. For more information, see [Adding Links to the Dashboard](admin-console-adding-buttons-links).

### Automatically Rewrite URLs Using KOTS

Although the primary purpose of the KOTS Application custom resource `ports` key is to port forward services for existing cluster installations, it is also possible to configure the `ports` key so that links to NodePort services for Embedded Cluster or kURL installations are automatically rewritten during deployment. This can be useful when developing and testing releases for your application because it is easier to access the application after installation compared to needing to provide a hostname or IP address for the link ahead of time.

For an example, see [Example: NGINX Application with ClusterIP and NodePort Services](/vendor/admin-console-port-forward#example-nginx-application-with-clusterip-and-nodeport-services) in _Port Forwarding Services with KOTS_.

### Firewall Requirements

To be able to access the Admin Console and the NodePort service, the firewall for the VM must allow HTTP traffic and allow inbound traffic to the port where the service is exposed from their workstation. Users can consult their cloud provider's documentation for more information about updating firewall rules.