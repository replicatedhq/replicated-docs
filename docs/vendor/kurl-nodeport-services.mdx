...
### Link to User-Supplied IPs/Hostnames {#user-supplied}

If your users will supply an IP address or hostname during installation (for example, if users must set up ingress or a load balancer on the KOTS Config screen), then you can configure a Kubernetes SIG Application custom resource in your release to link to the NodePort service.

You can use KOTS template functions to render the user-supplied IP address or hostname in the URL.

For more information, see [Adding Links to the Dashboard](admin-console-adding-buttons-links).

### Link to Ephemeral or Undetermined IPs/Hostnames Using KOTS

In most cases, you can create a link to NodePort services using the Kubernetes SIG Application custom resource and KOTS template functions to render a user-supplied IP address or hostname, as described in [Link to User-Supplied IPs/Hostnames](#user-supplied) above.

However, there are some cases where this might not be possible. For example:
* You do not want to require users to set up ingress or a load balancer and/or provide the IP address/hostname through the KOTS Config screen
* The hostname or IP address cannot be determined 

In these cases, you can use KOTS to rewrite the URL at the time of installation. This requires configuring both the Kubernetes SIG Application custom resource and the KOTS Application custom resource `ports` key. Note that typically, the KOTS `ports` key is used to port forward services in existing cluster installations. However, the `ports` key has logic that allows KOTS to dynamically rewrite URLs in the Kubernetes SIG Application custom resource during installation. This logic can be used to link to NodePort services for kURL or Embedded Cluster installations as well if the IP/hostname is undetermined or not user-supplied.
