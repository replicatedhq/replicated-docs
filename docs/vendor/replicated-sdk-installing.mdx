...
If you package your application as an umbrella chart with its own subcharts, see [Package an umbrella Helm chart](helm-install-umbrella-charts).
...
1. Save and promote the release to an internal-only channel used for testing.

1. Install the release in a development environment. For more information, see:
     * [Online Installation with Embedded Cluster](/embedded-cluster/v3/installing-embedded)
     * [Install with Helm](/vendor/install-with-helm)

1. In the installation environment, verify that you see the `replicated` Deployment:

    ```
    kubectl get deploy --namespace NAMESPACE
    ```
    Where `NAMESPACE` is the namespace in the cluster where you installed the SDK.

    **Example output**:

    ```
    NAME         READY   UP-TO-DATE   AVAILABLE   AGE
    your-app     1/1     1            1           35s
...
1. Download the `.tgz` chart archive for the SDK Helm chart:
    ...
    ```
    helm pull oci://proxy.replicated.com/library/replicated --version SDK_VERSION
    ```
