...
When the Replicated SDK is installed as a subchart, it creates Kubernetes resources (such as the `replicated` Deployment, Service, ServiceAccount, and Role) using the name `replicated`. To make these resources match your product or company branding rather than exposing the `replicated` name to end customers in the cluster, you can rename the SDK subchart by using Helm's [`alias`](https://helm.sh/docs/topics/charts/#tags-and-condition-fields-in-dependencies) field on the dependency.

The alias only changes the local name that Helm uses to reference the SDK subchart and the names of the resources it creates in the cluster. The chart is still pulled from `oci://registry.replicated.com/library/replicated`, and the SDK's telemetry, license enforcement, and API behavior are unchanged.

To brand the SDK with a Helm alias:

1. In your application Helm chart `Chart.yaml` file, add the `alias` field to the SDK dependency. The alias must be a valid Kubernetes resource name (lowercase alphanumeric characters and `-`).
...
1. Package your application Helm chart and install it as described in [Install the SDK as a subchart](#install-the-sdk-as-a-subchart) above and [Installing with Helm](/vendor/install-with-helm).

### Example result in the cluster

The following example shows the result of installing an application Helm chart named `myapp` that includes the SDK as a dependency aliased to `mycompany-sdk`. The SDK resources in the cluster are named after the alias instead of `replicated`:
