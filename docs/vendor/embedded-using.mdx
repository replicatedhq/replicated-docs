...
This section describes how to reset nodes with Embedded Cluster, including details about the Embedded Cluster `reset` command and best practices for resetting nodes.
...
1. Run safety checks. For example, `reset` will not remove a controller node when there are workers nodes available. And, it will not remove a node when the etcd cluster is unhealthy.
...
### Best Practices for Multi-Node Clusters

When resetting nodes in multi-node clusters in production environments, consider the following best practices to avoid issues:

* When deleting the entire cluster, run `reset` on each of the worker nodes first. Then, run `reset` on controller nodes. Controller nodes also remove thmselves from etcd membership.

* If you need to reset a controller node in a three-node cluster, first join a fourth controller node to the cluster before removing the target node. This ensures that you maintain a minimum of three nodes for the Kubernetes control plane. You can add and remove worker nodes as needed because they do not have any control plane components.
...
1. Run the following command to remove the node and reboot the machine:
    ...
    When resetting a single node or deleting a test environment, you can include the `--force` and `-y` flags to ignore any errors. For more information, see [reset](/reference/embedded-cluster-reset).
