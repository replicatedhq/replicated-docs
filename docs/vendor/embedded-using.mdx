...
An Embedded Cluster Config must be present in the application release to support installation with Embedded Cluster. At minimum, the Embedded Cluster Config sets the version of Embedded Cluster that will be installed. You can also define several characteristics about the cluster in the Config.

:::note
These steps assume that you have at least one release for your application that supports installation with Replicated KOTS or Replicated kURL. If you have not yet onboarded to Replicated, see [Onboard to the Replicated Platform](/vendor/replicated-onboarding) for more detailed instructions on how to create your first release.
:::

To add the Embedded Cluster cluster for your application:

1. Create a new release that includes your application and the Replicated [HelmChart v2](/reference/custom-resource-helmchart-v2) custom resource. If you have not yet created any releases for your application, see [Onboard to the Replicated Platform](/vendor/replicated-onboarding) for more detailed instructions on how to create your first release.

1. In the release, add an [Embedded Cluster Config](/reference/embedded-config) manifest. At minimum, the Config must include the Embedded Cluster version to use.

     <EcConfig/>

1. If your application requires that certain components are deployed before the application and as part of the Embedded Cluster itself, then update the Embedded Cluster Config to add [extensions](/reference/embedded-config#extensions).
...
1. Install with Embedded Cluster in a development environment to test:
     ...
     1. Under **Install types**, enable the **Embedded Cluster (current generation product)** option. Save the customer.
...
1. After successfully installing your application with Embedded Cluster, customize the [Embedded Cluster Config](/reference/embedded-config) as desired:
     ...
     * Define roles to assign workloads to specific nodes in multi-node installations. See [roles](/reference/embedded-config#roles).
     ...
     Replicated recommends that you work in small iterations and test your changes frequently by installing in development environments.

## (Optional) Serve Installation Assets Using the Vendor API

To install with Embedded Cluster, your end customers need to download the Embedded Cluster installer binary and their license. Air gap installations also require an air gap bundle.

Your end customers can download all these Embedded Cluster installation assets from the Replicated Enterprise Portal. No additional configuration is required. However, some vendors already have a portal where their customers can log in to access documentation or download artifacts. In cases like this, you can serve the Embedded Cluster installation essets yourself using the Replicated Vendor API, rather than having customers download the assets from the Replicated app service using a curl command during installation.
