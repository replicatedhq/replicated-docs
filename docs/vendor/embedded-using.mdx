...
When `--ignore-host-preflights` is passed, the built-in host preflight checks are still run, but the user is prompted and can choose to continue if failures occur. This ensures that users see any incompatibilities in their environment, while enabling them to bypass failures if necessary.
