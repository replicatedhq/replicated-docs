...
The reset command in Embedded Cluster performs a sequence of steps:
...
There will be some safety checks to run during the reset as well, such as:

     * No removal when the etcd cluster is not healthy
     * No removal of the controller node when worker nodes are still available

When cleaning up a single node or deleting a test environment, this command can be used with --force and -y flag.
For production clusters, make sure to run this on worker nodes first and leave the controller nodes until last if you want to delete the entire cluster, the controller node will also remove itself from etcd membership.
