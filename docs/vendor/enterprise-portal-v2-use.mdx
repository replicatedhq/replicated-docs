...
The Enterprise Portal does not generate step-by-step, in-console install or upgrade instructions for KOTS or kURL. Instead, these instance cards expand into a panel for downloading versioned artifacts. Customers select any release newer than their current version, or the current version when they are up to date, and download the artifacts for that release, such as the application air gap bundle, installer or admin console bundles, and any available command-line tools.

The download panel appears only for customers whose license has the KOTS or kURL install type together with air gap enabled. If the customer's license is expired, the panel shows a renewal notice instead of the version picker.
...
The instance cards, including the KOTS and kURL download panel, are rendered by the `<InstancesAndUpdates />` component in the default content template. For more information, see [MDX components](/vendor/enterprise-portal-v2-content#mdx-components) in _Customize Portal Content_.
