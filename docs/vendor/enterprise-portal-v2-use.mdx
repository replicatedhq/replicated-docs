...
For online and proxy instances, the panel gives Admin Console guidance without downloading an air gap artifact. For air gap instances, it provides the selected release's application air gap bundle and directs the customer to upload it from **Version History** in Admin Console. An expired license or a license without air gap access does not expose the bundle.
