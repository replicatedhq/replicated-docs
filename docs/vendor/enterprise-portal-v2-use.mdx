...
For online and proxy instances, the panel gives Admin Console guidance without downloading an air gap artifact. For air gap instances, it provides the selected release's application air gap bundle. When that bundle is ready and the instance is not already on the release, the panel directs the customer to upload it from **Version History** in Admin Console and deploy. A bundle that is still building stops at that status, and a card for the installed release does not say to deploy. An expired license or a license without air gap access does not expose the bundle.
