...
For online and proxy instances, the panel gives Admin Console guidance without downloading an air gap artifact. For air gap instances, it provides the selected release's application air gap bundle. When that bundle is ready and the instance is not already on the release, the panel includes `kubectl kots admin-console -n <app>` to reopen the Admin Console, then directs the customer to upload the bundle from **Version History** and deploy. A bundle that is still building stops at that status, and a card for the installed release does not say to deploy. An expired license or a license without air gap access does not expose the bundle.
