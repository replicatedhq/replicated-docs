...
**SAML SSO:** When SAML is configured by the vendor and the customer's team admin, customers can log in using their corporate identity provider credentials. On a first SAML login, the portal creates the account automatically. A login that starts at the identity provider needs no invitation. A login that starts at the portal login page requires a pending invitation for that email address. For more information, see [Just-in-time user provisioning](/vendor/enterprise-portal-v2-invite#jit-provisioning) in _Manage Customer Access_.
...
For upgrades, customers use the **Instances & Updates** page. Helm and Embedded Cluster instance cards with an available update expand into generated CLI upgrade steps. KOTS and kURL cards expand into guided Admin Console application-upgrade steps. The portal identifies the selected target and supplies an application air gap bundle when required, but the customer performs and monitors the deployment in Admin Console.

Instances that are already on the latest version show an **Up to date** badge. Customers can still expand supported cards to review the current release's commands or Admin Console guidance.
...
KOTS and kURL instance cards expand into a guided application-upgrade panel. Customers can review or change the intended target release and then follow ordered steps for checking, selecting, and deploying that release in Admin Console. The panel does not show an unverified upgrade CLI command or track deployment completion.

For online and proxy instances, the panel tells the customer that Version History is a tab in the Admin Console, includes `kubectl kots admin-console -n <app>` to reopen it, and tells them to click Check for updates before deploying. It does not download an air gap artifact. For air gap instances, it provides the selected release's application air gap bundle. When that bundle is ready and the instance is not already on the release, the panel includes `kubectl kots admin-console -n <app>` to reopen the Admin Console, then directs the customer to upload the bundle from **Version History** and deploy. A bundle that is still building stops at that status, and a card for the installed release does not say to deploy. An expired license or a license without air gap access does not expose the bundle.

For kURL, this panel upgrades the application only. It does not imply that every application release requires rerunning `install.sh` or upgrading the Kubernetes cluster. Vendors document any separate cluster-maintenance conditions and procedure in the customizable kURL upgrade guide.

The default content template includes dedicated KOTS and kURL install pages and entitlement-gated application-upgrade guides. Vendors can customize Admin Console access, registry, support, and kURL cluster-maintenance instructions, change the panel's guide link, or remove it. For installation-page details, see [KOTS and kURL installer pages](/vendor/enterprise-portal-v2-content#add-kots-or-kurl-installer-pages) in _Customize Portal Content_.
...
When Security Center access is enabled for a customer, customers with Helm or Embedded Cluster installations can view security data for their releases.
