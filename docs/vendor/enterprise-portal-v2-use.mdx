...
- **CVE reports**: Browse vulnerabilities by severity, component, and container image for the selected release. The **Fixed In** column identifies the earliest subsequent release on the customer's channel where the CVE is no longer detected. A dash indicates that no release-level fix is known.
