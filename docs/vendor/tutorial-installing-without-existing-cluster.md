...
When you are familiar with these concepts, you can use the replicated CLI and API to automate this task rather than manually editing the YAML on this page. For more information, see [Installing the replicated CLI](replicated-cli-installing) and [Using the Replicated Vendor API v3](../reference/vendor-api-using).
