...
1. In `manifests`, open the HelmChart custom resource that you configured in a previous step. Configure the `values` key of the HelmChart custom resource to map the fields in the KOTS Config custom resource to your Helm values. For more information, see [`values`](/reference/custom-resource-helmchart-v2#values) in _HelmChart v2_
