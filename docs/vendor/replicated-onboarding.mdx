...
1. In `kots-config.yaml`, add the KOTS Config custom resource. Configure the KOTS Config custom resource based on the values that you need to collect from users.
    ...
    * [Config](/reference/custom-resource-config)
    ...
    <details>
    <summary>Can I view a preview of the Admin Console config screen before installing the release?</summary>

     The Vendor Portal includes a **Config preview** pane on the **Edit release** page that allows you to preview the Admin Console config screen without having to install the release.

     To view the preview, click **Config preview** in the pane on the right side of the **Edit release** page. For example:
...
Before distributing your application to customers, ensure that your documentation is up-to-date. In particular, be sure to update the installation documentation to include the procedures and requirements for installing with Embedded Cluster, Helm, and/or with KOTS in an existing cluster. 
