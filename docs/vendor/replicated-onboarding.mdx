...
:::note
For vendors with access to the Replicated KOTS installer that do _not_ intend to distribute an application that is packaged with Helm, see [KOTS Tutorial (UI)](tutorial-ui-setup) to follow an onboarding workflow that uses a sample application with standard Kubernetes manifests and demonstrates installing with Replicated KOTS.
::: 
