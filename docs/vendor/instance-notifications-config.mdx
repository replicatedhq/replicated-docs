...
:::caution
The Instance Notifications (Classic) system is replaced by [Event Notifications (Beta)](event-notifications), which offers more event types, better filtering, and improved delivery options. While Instance Notifications (Classic) is still supported, Replicated recommends using Event Notifications (Beta). For more information about how to configure event notifications, see [About Event Notifications (Beta)](event-notifications).
:::
