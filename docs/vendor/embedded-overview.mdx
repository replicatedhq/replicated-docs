...
Embedded Cluster uses disk space in the following locations:

* `/var/lib/embedded-cluster` stores Embedded Cluster binaries, logs, and temporary files.
* `/var/lib/k0s` stores the Kubernetes configuration, data, and container images.
* `/var/openebs` stores PVC data.

Replicated does not recommend using symlinked directories for these locations.
