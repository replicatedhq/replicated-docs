...
* **Upgrading by more than one Kubernetes version at a time not supported**: Kubernetes does not support upgrading by more than one minor version at a time. If end-users attempt to upgrade to a release that specifies a Kubernetes version two or more minor versions later than what is currently installed, the Admin Console blocks the upgrade with the following message: "INSERT ERROR MESSAGE HERE". To prevent end-users from attempting to skip a minor version of Kubernetes, Replicated recommends that you select **Prevent this release from being skipped during upgrades** when you promote a new application release that increases the Kubernetes version. For more information about required releases, see [Properties](/vendor/releases-about#properties) in _About Channels and Releases_.
