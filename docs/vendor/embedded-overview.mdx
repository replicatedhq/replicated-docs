...
* **Custom domains not supported**: Embedded Cluster doesn't fully use custom domains even if they're configured. Full support for custom domains will be added.

* **KOTS Auto-GitOps workflow not supported**: Embedded Cluster does not support the KOTS Auto-GitOps workflow. If an end-user is interested in GitOps, consider the Helm install method instead.
