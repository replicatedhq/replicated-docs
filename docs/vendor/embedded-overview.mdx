...
Air Gap installations function almost identically to online installations, but require an airgap bundle unique to your application and version.

With that bundle, you can install your application in an airgapped environment using the same steps as an online installation:

    ```
    sudo ./APP_SLUG install --license LICENSE_FILE --airgap-bundle AIRGAP_BUNDLE
    ```

Air Gap installations provide the same functionality as online installations, including updates, adding nodes, and resetting nodes.
A docker registry will be run within the cluster when using an airgap bundle and used to store application images and infrastructure files.
Online (non-airgap) installations instead pull images from the public internet.
