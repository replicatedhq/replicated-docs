...
The following flags can be used to install the Embedded Cluster behind a proxy server:
...
<table>
  ...
  <tr>
     <th>--http-proxy</th>
     <td>HTTP proxy to use for the installation</td>
  </tr>
  <tr>
     <th>--https-proxy</th>
     <td>HTTPS proxy to use for the installation</td>
  </tr>
  <tr>
     <th>--no-proxy</th>
     <td>Comma separated list of hosts to bypass the proxy for</td>
  </tr>
</table>
...
```bash
sudo ./APP_SLUG install --license LICENSE_FILE \
--http-proxy=172.31.12.157:3128 \
--https-proxy=172.31.12.157:3128 \
--no-proxy=localhost,127.0.0.1,.default,.local,.svc,kubernetes,kotsadm-rqlite,172.31.0.0/16,10.0.0.0/8
```

In this example, `172.31.12.157:3128` is the location of the proxy server. For the `--no-proxy` flag, we configure the following:
1. A set of defaults, to avoid cluster internal communication to go over the proxy: `localhost,127.0.0.1,.default,.local,.svc,kubernetes`
2. Kots specific host allowing to reach the database: `kotsadm-rqlite`
3. Host Node to Node communication: `172.31.0.0/16`
4. Kubernetes Cluster Pod and service CIDR: `10.0.0.0/8`

In the case of using the Embedded Cluster in a multinode installation, the proxy server settings will automatically be configured when joining additional nodes.

Limitations:
* If the applications requires proxy settings, those will not be automatically propagated and have to be manually configured.
* The proxy settings can not be changed after installation or during upgrade.
