...
* **Multi-node support is alpha**: Support for multi-node embedded clusters is in alpha. Only single-node embedded clusters are in beta.

* **No migration from kURL**: There is not a supported path to migrate an existing kURL instance to Embedded Cluster.

* **Air gap support is in alpha**: Support for air gap installations with Embedded Cluster is an alpha feature.

* **Monitoring applications with Prometheus not supported**: Embedded Cluster installations do not include Prometheus and therefore do not display graphs on the Admin Console dashboard for the purpose of monitoring the instance. For more information about monitoring applications with Prometheus in existing cluster or kURL installations, see [Monitoring Applications](/enterprise/monitoring-applications).

* **Backup and restore with snapshots is not supported**: Embedded Cluster installations do not support backup and restore with the KOTS snapshots feature. For more information about snapshots, see [Understanding Backup and Restore](/vendor/snapshots-overview).

* **Rollbacks not supported**: The [`allowRollback`](/reference/custom-resource-application#allowrollback) field in the KOTS Application custom resource is not supported for Embedded Cluster. In Embedded Cluster installations, the application and the cluster are installed and updated together as a single appliance. Because of this, users cannot roll back (downgrade) the application to an earlier version.
...
* **KOTS KubernetesVersion template function not supported**: The [KubernetesVersion](/reference/template-functions-static-context#kubernetesversion) template function does not provide accurate Kubernetes version information for Embedded Cluster installations. This is because template functions are rendered during deployment, before any necessary Kubernetes updates are completed. Vendors can know the version of Kubernetes that is deployed in a cluster based on the version of Kubernetes that is included in the corresponding Embedded Cluster config. 
