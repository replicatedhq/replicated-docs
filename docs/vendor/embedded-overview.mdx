...
When resetting a node, OpenEBS PVCs on the node are deleted. If a PVC is created by your application directly and not as part of a StatefulSet, it will not be recreated automatically on a remaining node. You will need to redeploy your application for the PVC to be recreated.
