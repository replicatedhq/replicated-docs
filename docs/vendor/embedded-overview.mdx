...
Embedded Cluster stores data in three primary locations, all of which must be writable:

#### 1. `/var/lib/embedded-cluster`

- Used to store your installer
- Includes:
  * Logs
  * Temporary files
  * Supporting binaries (e.g., kubectl and troubleshoot)
...
- Storage requirements:
  * The total space should be sufficient to hold the following while remaining below 85% utilization:
    1. All pod logs
    2. Two copies of infrastructure images included in k0s
    3. All pod images for your application
...
- For airgap installs:
  ...
  * If the `--enable-ha` flag is used all control planes will have the same requirements, as data will be replicated to the other control planes.

#### 3. `/tmp`
- Temporary space during installation

**Note:** Replicated does not support using symlinked directories for these locations.
