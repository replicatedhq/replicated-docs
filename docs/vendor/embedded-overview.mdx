...
* **Multi-node support is in alpha**: Support for multi-node embedded clusters is in alpha. Only single-node embedded clusters are in beta. For more information, see [Enable High Availability](#) below.

* **Air gap support is in alpha**: Support for air gap installations with Embedded Cluster is an alpha feature. For more information, see [Embedded Cluster](/vendor/embedded-air-gap).
