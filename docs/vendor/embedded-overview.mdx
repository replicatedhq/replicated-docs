...
* **Partial rollback support**: Rollbacks for Embedded Cluster installations are only supported to a version which there is no change to the [Embedded Cluster Config](https://docs.replicated.com/reference/embedded-config) spec from the currently installed version.
