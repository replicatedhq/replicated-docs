...
**Limitations:**
* Proxy settings are not automatically propagated to the containers for your [Helm extensions](/reference/embedded-config#helm-extensions) and must be manually configured (typically via Helm values). The images for your Helm extensions will be pulled through the specified proxy.
