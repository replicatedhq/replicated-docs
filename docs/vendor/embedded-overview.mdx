...
* **Multi-node support is in alpha**: Support for multi-node embedded clusters is in alpha. Only single-node embedded clusters are in beta.
...
* **Monitoring applications with Prometheus not supported**: Embedded Cluster installations do not include Prometheus to collect instance and cluster metrics. Embedded Cluster installations also do not display graphs on the Admin Console dashboard for monitoring the instance, nor support adding custom graphs to the Admin Console dashboard using the KOTS Application custom resource [`graphs`](/reference/custom-resource-application#graphs) field. For more information about monitoring applications with Prometheus in existing cluster or kURL installations, see [Monitoring Applications](/enterprise/monitoring-applications).
