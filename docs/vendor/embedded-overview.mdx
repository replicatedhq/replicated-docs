...
After all the Kubernetes components for the cluster are available, the Embedded Cluster binary then installs the Embedded Cluster add-ons.

Embedded Cluster add-ons include:
* **KOTS:** Embedded Cluster deploys the KOTS Admin Console in the kotsadm namespace. KOTS deploys the application and a rqlite StatefulSet. KOTS uses rqlite to store the version history, application metadata and other small amounts of data needed to manage the application. Rqlite is a distributed relational database that uses SQLite as its storage engine. For more information, see the [rqlite](https://rqlite.io/) website.
* **OpenEBS:** Embedded Cluster uses OpenEBS to provide local PersistentVolume (PV) storage for support bundles and application archives.
