...
* **Multi-node support is in alpha**: Support for multi-node embedded clusters is in alpha. Only single-node embedded clusters are in beta. For more information, see [Enable High Availability for Multi-Node Clusters](#ha) below.

* **Air gap support is in alpha**: Support for air gap installations with Embedded Cluster is an alpha feature. For more information, see [Installing with Embedded Cluster in Air Gap Environments](/vendor/embedded-air-gap).

* **Disaster recovery is in alpha**: Disaster Recovery for Embedded Cluster installations is in alpha. For more information, see [Disaster Recovery for Embedded Cluster](/vendor/embedded-disaster-recovery)
...
1. Set up a cluster with at least two controller nodes. You can do an online (internet-connected) or air gap installation. For online clusters, see [Add Nodes (Alpha)](#add-nodes) above. For air gap clusters, see [Air Gap Installations with Embedded Cluster (Alpha)](/enterprise/installing-embedded-air-gap).

1. SSH onto a third node that you want to join to the cluster as a controller.

1. Run the join command and pass the `--enable-ha` flag.
...
* In the future, the prompt about migrating to high availability will display automatically if the cluster isn’t yet HA and you’re adding the third or more controller node. The `--enable-ha` flag serves as a feature flag during the Alpha phase.
