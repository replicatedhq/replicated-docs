...
import StorageReqs from "../partials/embedded-cluster/_storage-reqs.mdx"
...
Embedded Cluster supports installations in both online (internet-connected) environments and air gap environments with no outbound internet access. For online installations, Embedded Cluster also supports installing behind a proxy server.
...
* [Installing in Air Gap Environments with Embedded Cluster (Alpha)](/enterprise/installing-embedded-air-gap)

### (Optional) Serve Installation Artifacts Using the Vendor API

To install with Embedded Cluster, you need to download an Embedded Cluster binary and a license. Your customers can download these installation artifacts using the Replicated app service by running a curl command. Alternatively, you can host/serve the binary yourself using the Replicated Vendor API.

Some vendors already have a portal where their customers can log in to access documentation or download artifacts. In cases like this, you can serve the Embedded Cluster installation artifacts yourself using the Replicated Vendor API.
