...
* The Embedded Cluster installation assets include the customer's license and the Embedded Cluster binary. For air gap installations, an air gap bundle containing the images required to run the application in environments with limited or no outbound internet access is also included.

* In the primary node, the Embedded Cluster binary installs the cluster So k Zero's cluster runs as a system d service deploys a systemd service. This first systemd service is named using the unique slug of the application (such as gitea). This service The binary installs the cluster, so it deploys that system. It starts that system d service and then waits for the cluster to start. So it waits for all the different Kubernetes components to come up and be available. And then it's going to. I believe the order is that it's then going to install the Add-ons. 

* After the first systemd service is deployed, the Embedded Cluster binary then deploys a second systemd service (named APP_SLUG-manager). This manager service orchestrates Embedded Cluster and communicates with the KOTS instance running in the cluster through a websocket.

* Any optional Helm extensions that are included in the Embedded Cluster Config are also deployed in the cluster in one or more namespaces defined by the vendor. The diagram above shows two different Helm extensions deployed in the cluster, each in its own namespace.
