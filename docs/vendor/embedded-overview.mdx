...
In this example, `172.31.12.157:3128` is the location of the proxy server. And, the `--no-proxy` flag configures the following:
* A set of defaults to avoid cluster internal communication to go over the proxy: `localhost,127.0.0.1,.default,.local,.svc,kubernetes`
* KOTS-specific host allowing to reach the database: `kotsadm-rqlite`
* Host Node-to-Node communication: `172.31.0.0/16`
* Kubernetes Cluster Pod and service CIDR: `10.0.0.0/8`

When using Embedded Cluster in a multinode installation, the proxy server settings are automatically configured when joining additional nodes.

**Limitations:**
* Proxy settings are not automatically propagated and must be manually configured.
