...
For an existing customer, use [Update a customer](https://replicated-vendor-api.readme.io/reference/updatecustomer-1) to set `is_enterprise_portal_enabled` to `true`.
The [Create a customer](https://replicated-vendor-api.readme.io/reference/createcustomer-1) request accepts `portal_version: "v2"` for a new customer.
For an existing customer, use [Update Enterprise Portal settings](https://replicated-vendor-api.readme.io/reference/updateenterpriseportalsettings) with `customerId` and `portalVersion: "v2"` to select the New Enterprise Portal.
