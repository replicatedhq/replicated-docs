...
* To collect telemetry from an air gap instance, one of the following must be installed in the cluster where the instance is running:
   
   * The Replicated SDK installed in _air gap mode_. See [Installing the SDK in Air Gap Environments](/vendor/replicated-sdk-airgap).
   
   * KOTS v1.92.1
   
   :::note
   If both the SDK and KOTS v1.92.1 are installed in the cluster (such as when a Helm chart that includes the SDK is installed by KOTS), then both collect and store instance telemetry.
   :::

* The Replicated SDK is required to collect custom metrics from air gap instances.
