...
To configure your application to use the proxy registry for installations with Embedded Cluster v3:
...
In addition to being the supported path going forward, the proxy registry serves images from a globally-distributed edge network rather than from a single region. For more information, see [About image delivery and caching](/vendor/private-images-about#delivery).
