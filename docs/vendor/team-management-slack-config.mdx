...
:::caution
This Slack webhook configuration is part of Instance Notifications (Classic), which is being replaced by [Event Notifications (Alpha)](event-notifications). The new Event Notifications system supports direct webhook delivery to Slack (and other services) without requiring team-level Slack configuration.

For new integrations, Replicated recommends using [Event Notification Webhooks (Alpha)](event-notifications-webhooks) instead.
:::
