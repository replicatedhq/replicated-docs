...
:::caution
This Slack webhook configuration is part of Instance Notifications (Classic). [Event Notifications](event-notifications) replaces this system. The new Event Notifications system supports direct webhook delivery to Slack (and other services) without requiring team-level Slack configuration.

For new integrations, Replicated recommends using [Configure and test webhooks](event-notifications-webhooks) instead.
:::
