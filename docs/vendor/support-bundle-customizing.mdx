...
1. Update `spec.collectors` with additional collectors based on your application needs. For a list of all available collectors, see [All Collectors](https://troubleshoot.sh/docs/collect/all/) in the Troubleshoot documentation.
    ...
    :::important
    Do not write credentials into a collector spec as literal values, including values supplied by Helm template functions or KOTS config options. The rendered spec is deployed to the cluster, where a support bundle can collect it in plain text. For the recommended approach, see [Run a pod with a credential from a Secret](/vendor/support-bundle-examples#run-pod-credentials) in _Example support bundle specs_.
    :::
