...
1. Run the installation script from above with the following changes to deploy the admin console at the same time you install the app manager. In this step, the license file uploads and the preflight checks run as part of the deployment:
  ...
  • Go to http://localhost:8888 to access the application
   ...
   :::note
   If you enable port forwarding, the kots CLI output includes the URLs where you can access the application and the admin console. For more information about each of the flags for the `kots install` command, see [install](/reference/kots-cli-install) in the kots CLI documentation.
   :::
