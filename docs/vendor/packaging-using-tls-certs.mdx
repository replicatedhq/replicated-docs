...
Replicated KOTS provides default self-signed certificates that renew automatically. 

For Replicated kURL installations, the self-signed certificate renews 30 days before expiration when you enable the kURL EKCO add-on version 0.7.0 and later.

End customers can also upload a custom TLS certificate in kURL installations. kURL creates a TLS secret that can reused by other Kubernetes resources, such as Deployment or Ingress, which can be easier than providing and maintaining multiple certificates. As a vendor, you can enable the use of custom TLS certificates with these additional resources.
