...
To use the proxy registry in Helm CLI installations, update your Helm values so that image references point to the Replicated proxy registry rather than to your default registry.
...
1. <Helper/>
   ...
   :::note
   The `enterprise-pull-secret` is automatically created by the Replicated SDK. If you want to create your own image pull secret to authenticate with the Replicated proxy registry, such as if you manage the pull secret externally, see [(Optional) Create Your Own Image Pull Secret](#create-pull-secret).
   :::
