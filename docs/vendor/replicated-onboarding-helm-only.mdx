...
1. In the `manifests` directory, create a new YAML file named `YOUR_CHART_NAME.yaml`. For example, `samplechart.yaml`. In the file, add the following YAML to create a HelmChart custom resource. Update the values to match the name and version of the corresponding Helm chart.
...
### Task : Alias Replicated Endpoints With Your Own Custom Domains {#custom-domains}

<CustomDomains/>

For more information about updating image references, see [Use the Proxy Registry with Helm CLI Installations](/vendor/helm-image-registry).
...
1. Follow the steps in [Install and Update with Helm in Air Gap Environments](/vendor/helm-install-airgap) to access the Enterprise Portal for the customer and test.

    :::note
    Replicated recommends that you use Compatibility Matrix (CMX) to create an air-gapped environment for testing the release. For more information, see [Test in Air Gap Environments](/vendor/testing-network-policy).
    :::
...
### Add Support for Replicated Embedded Cluster Installations
