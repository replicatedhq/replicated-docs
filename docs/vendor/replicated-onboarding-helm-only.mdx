...
For information about how to onboard your application to the Replicated Platform to support installations with Replicated installers and the Helm CLI, see [Onboard to the Replicated Platform](replicated-onboarding).
...
Complete the tasks in this section to onboard your application to the Replicated Platform. When you are done, you can continue to the [Next Steps](#next-steps) to integrate other Replicated features with your application.
...
To use the proxy registry in Helm CLI installations, update your Helm values so that image references point to the Replicated proxy registry rather than to your default registry. Then, provide authentication for the proxy registry by including the `enterprise-pull-secret` image pull secret in any image references. The `enterprise-pull-secret` is automatically created by the Replicated SDK. For more information about this process, see [Use the Proxy Registry with Helm CLI Installations](/vendor/helm-image-registry).
...
1. <Helper/>
   ...
   :::note
   The `enterprise-pull-secret` is automatically created by the Replicated SDK. If you want to create your own image pull secret to authenticate with the Replicated proxy registry, such as if you manage the pull secret externally, see [(Optional) Create Your Own Image Pull Secret](/vendor/helm-image-registry#create-pull-secret).
   :::
...
1. In the `manifests` directory, create a new YAML file named `YOUR_CHART_NAME.yaml`, where `YOUR_CHART_NAME` is the name of your chart. For example, `samplechart.yaml`.

1. In the `YOUR_CHART_NAME.yaml` file, add the following YAML to create a HelmChart custom resource. Update the values for `chart.name` and `chart.chartVersion` to match the name and version of the corresponding Helm chart.
...
1. If your application is deployed as multiple Helm charts, repeat the previous steps to add a unique HelmChart custom resource for each Helm chart archive in the release.
...
### Task 6: Define preflight checks
