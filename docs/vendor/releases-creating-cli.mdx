...
1. (Helm Charts Only) If your release contains any Helm charts:
   ...
   1. Package the Helm charts and add the `.tgz` files to the local directory with your application files. See [Creating a Helm Chart Package](helm-release-creating-package).

   1. (KOTS Only) If your release supports installation with Replicated KOTS, configure a HelmChart custom resource for each Helm chart in your release and add them to the local directory with your application files. See [HelmChart v2](/reference/custom-resource-helmchart-v2).
