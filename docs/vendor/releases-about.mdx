...
* In the **Latest release** section, view the properties of the latest release, and get information about any warnings or errors in the YAML files for the latest release.
   ...
   Click **Release history** to access the history of all releases promoted to the channel.
...
The **Release History** page lists all releases promoted to the selected channel. Use the view toggle to switch between the following views:

* **Table**: The default view. The Table view displays release metadata and actions. From this view, you can view the version labels and files in each release, build and download air gap bundles for use with Replicated installers (Embedded Cluster, KOTS, kURL), edit release properties, and demote releases from the channel.

  The following shows the Table view:

  <img src="/images/channels-release-history.png" alt="Table view on the Channel Release History page" width="750"/>

  [View a larger version of this image](/images/channels-release-history.png)

* **Changelog**: Displays release notes as rendered Markdown cards across the releases on the channel, so that you can read the notes without opening each release individually. Each card identifies the release and includes its promotion or demotion date, status badges, compatible install types, and an **Edit** action. For release notes that exceed the available space on the card, click **Show more** to expand the notes and **Show less** to collapse them.

  The following shows the Changelog view:

  <img src="/images/release-history-changelog.png" alt="Changelog view on the Channel Release History page" width="750"/>

  [View a larger version of this image](/images/release-history-changelog.png)

  If none of the releases on the page have release notes, the empty state explains that you can add notes by choosing **Edit release** for an entry in the Table view. Click **View releases** to return to the Table view.
