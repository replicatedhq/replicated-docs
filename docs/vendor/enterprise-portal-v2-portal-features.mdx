...
| **Show only fixable CVEs in Security Center** | Enabled | Limits the CVE report to vulnerabilities with an available fix. Disable it to let customers switch between all CVEs and fixable CVEs. See [Security Center display settings](#security-center-display-settings). |
| **Allow raw CVE scan downloads in Security Center** | Disabled | Lets customers download the raw Grype scan JSON for a release. See [Security Center display settings](#security-center-display-settings). |
...
The **Show only fixable CVEs in Security Center** and **Allow raw CVE scan downloads in Security Center** settings control how Security Center reports appear to customers. They do not grant access to the Security Center itself.
