...
It can be useful to show or hide configuration fields if the distribution of the cluster is Replicated embedded cluster because embedded cluster distributions include built-in extensions to manage functionality such as ingress and storage. This means that embedded clusters frequently have fewer configuration options for the user.
