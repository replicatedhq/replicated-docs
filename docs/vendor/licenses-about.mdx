...
## About Licenses Updates

### Updating Licenses

You can make changes to a customer in the Vendor Portal to edit their license details, including the license type or the customer name, at any time. The license ID, which is the unique identifier for the customer, never changes. For more information about how to edit licenses, see [Edit a Customer](releases-creating-customer#edit-a-customer) in _Creating and Managing Customers_.

You can also update licenses to make changes to a customer's entitlements, such as changing a license field's value, adding new fields, or removing fields.

The Replicated SDK polls the Vendor Portal for any license updates every four hours. Any changes to the license (such as changes to a license field value, new fields, or removed fields) are reflected. The user does not need to take any action to apply the updated license.

For KOTS installations, users can sync their license from the Admin Console. Applying any changes to the license requires that the user deploys a new version. For more information about how licenses can be updated from the Admin Console, see [Updating Licenses](/enterprise/updating-licenses).
