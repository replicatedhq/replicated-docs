...
_Embedded Kubernetes_ refers to distributing Kubernetes alongside your application in order to support KOTS installations on VMs or bare metal servers. During installation, a cluster is provisioned in the customer environment where KOTS and the application are then deployed. Embedding Kubernetes with your application benefits enterprise users that do not have an existing cluster, do not have experience with Kubernetes, or otherwise prefer to install on a VM or bare metal server.
