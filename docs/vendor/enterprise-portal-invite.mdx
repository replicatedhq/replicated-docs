...
You can enable the Enterprise Portal for all customers at once, or on a per-customer basis. When the Enterprise Portal is disabled for a customer, they have access to the Replicated Download Portal instead.
...
Do one of the following to manage Enterprise Portal access for your customers:

* Go to **Enterprise Portal > Customer Access**. In the **Portal Access** section, enable the **Enable Enterprise Portal for all customers** toggle to route all customers to the Enterprise Portal. When this global setting is enabled, all per-customer toggles are disabled and display context about the active override.
...
* Go to **Customers** and select the target customer. On the customer's page, go to **Enterprise Portal access** and enable the **Enable Enterprise Portal for this customer** toggle to route that customer to the Enterprise Portal.

   When the global **Enable Enterprise Portal for all customers** setting is on, the per-customer toggle is disabled. An info banner explains that the global setting is active and shows what the customer's individual setting would be if the global override were turned off.
