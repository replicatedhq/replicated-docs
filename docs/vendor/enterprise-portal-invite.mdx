...
When the Enterprise Portal is enabled globally, you can configure automatic invitations so that new customers receive an Enterprise Portal invite as soon as they are created. This removes the need to manually navigate to the customer's Enterprise Portal access tab after creation.
...
You can override the app-level auto-invite setting on a per-customer basis during customer creation. On the **Create a new customer** page, use the **Send Enterprise Portal invite to customer email** checkbox to send or skip the invite for that specific customer. For more information, see [Create a Customer](releases-creating-customer#create-a-customer).
...
:::note
You can also configure automatic invitations so that new customers are invited to the Enterprise Portal when they are created, without any additional steps. See [Automatically Invite Customers on Creation](#auto-invite).
:::
