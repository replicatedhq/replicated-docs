...
:::note
Looking for the new Enterprise Portal? See [Manage customer access](/vendor/enterprise-portal-v2-invite) in the Enterprise Portal (New) section.
:::
...
The allowed domain list also governs SAML logins. The Enterprise Portal does not create an account for a user whose email domain is absent from the list, including a user who logs in through their IdP. The check applies only at account creation, so removing a domain does not lock out users who were already provisioned from it. For more information, see [About SAML logins](enterprise-portal-use#about-saml) in _Log In and Use the Enterprise Portal_.
...
## Enable SAML authentication {#enable-saml}

You can enable and disable SAML authentication for the Enterprise Portal on a per customer basis. When SAML authentication is enabled, the customer can set up SAML SSO logins for the Enterprise Portal using their identity provider (IdP). When SAML authentication is disabled, Enterprise Portal users are not able  to log in using SAML, even if the customer had already configured SAML for their Enterprise Portal previously. For more information, see [About SAML Logins](enterprise-portal-use#about-saml) in _Log In and Use the Enterprise Portal_.
...
1. On the customer's page, go to **Enterprise Portal access**. In the **Authentication** section, enable the **SAML Authentication** toggle.
   ...
   After you enable SAML authentication, the customer can configure SAML in the Enterprise Portal using their IdP. For more information, see [Configure SAML Authentication](/vendor/enterprise-portal-use#saml) in _Log In and Use the Enterprise Portal_.
