...
You can send an email notification to Enterprise Portal users when a new release is promoted, letting customers know an update is available. Notifications are off by default and must be explicitly opted into per release using the `--notify-users` CLI flag or the `notify-users` GitHub Actions parameter. Only customers with Enterprise Portal access receive the email.

For more information and examples, see [Notify Enterprise Portal users](/vendor/releases-creating-cli#notify-users).
