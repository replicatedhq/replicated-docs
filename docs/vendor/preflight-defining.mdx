...
Replicated recommends that you design these preflight checks around a pre-existing Secret. The Secret must already exist in the cluster when preflight checks run, so add a values field such as `existingSecret` that the customer sets to the name of a Secret that they create before they install your application. Document the keys that your preflight checks expect, such as `password`.
...
For Embedded Cluster installations, you can also create the Secret in a Helm chart extension, because extensions install before application preflight checks. For more information, see [Deployment pipeline](/embedded-cluster/v3/embedded-overview#deployment-pipeline).
...
The following example checks a PostgreSQL connection using a password from a pre-existing Secret. The customer supplies the name of the Secret in `.Values.global.externalDatabase.existingSecret`.
