...
:::note
In Embedded Cluster v3, required checks are honored in Embedded Cluster TKVERSION and later. The installer labels the failure **Required**, does not offer **Continue Anyway**, and rejects the `--ignore-app-preflights` and `--ignore-host-preflights` flags for that check. For more information, see [Whether users can bypass a failed check](/embedded-cluster/v3/embedded-using#preflight-bypass).
:::
