...
In Embedded Cluster v3, the installer labels a failed required check **Required** and does not offer **Continue Anyway**. The `--ignore-app-preflights` and `--ignore-host-preflights` flags do not override a required check. For more information, see [Whether users can bypass a failed check](/embedded-cluster/v3/embedded-using#preflight-bypass).
