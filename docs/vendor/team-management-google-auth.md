...
As a team administrator, you can enable, disable, or require Google authentication for all accounts in the team. You may also allow all users from your Google Workspace to automatically join your team.
...
1. Enable or disable the settings:
  ...
  | Field                  | Instructions           |
  |-----------------------|------------------------|
  | Allow Google Authentication for team members | Enable this setting to allow team members to log in using a Google account. |
  ...
  | Allow all users from my Google Workspace to be added to the team | When enabled, users within the same Google Workspace associated with the current team's domain will automatically be added to the team when logging in or signing up with Google Authentication. |
...
## Allowing all users from a Google Workspace to be added to the team
To enable this feature, an administrator must be logged in via Google Authentication. The Google Workspace account used when logging in via Google Authentication will be listed as the teams domain; this field is not editable and a Google Workspace domain may only be joined to a single vendor portal team. Administrators may select a default RBAC policy to be assigned to users who join from this method; by default, this field is set to the built-in read-only policy.
