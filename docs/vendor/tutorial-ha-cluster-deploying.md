...
The diagram shows four virtual machines (VMs) running on Google Cloud Platform (GCP), with one of them used solely as a load balancer. Note that there are not any requirements to use GCP. You can follow along with VMs provisioned on a different cloud provider or in an on-premise environment.
...
- This exercise requires four VMs that will need to communicate with each other. For more information about system requirements, see [Procuring the (virtual) hardware](#provisioning-the-virtual-hardware).
...
The application we will use in this guide is available in the [kotsapps repository](https://github.com/replicatedhq/kotsapps/tree/master/app-direct).
...
#### Adding EKCO to the Kubernetes installer
...
1. In your Kubernetes installer, add the following under spec:
   ...
   ```
2. Promote the installer to same channel(s) where you have promoted your application releases.
...
1. [Run the app manager installer on the first node and deploy the application](#running-the-kots-installer-and-deploying-an-application)
...
Like with the HAProxy VM, keep the [the app manager System Requirements](https://kurl.sh/docs/install-with-kurl/system-requirements) in mind when provisioning the VMs that will form the cluster. The command below creates a VM that meets those requirements, at least for testing purposes.
...
```
To create the remaining nodes, run the same command but increment the node number (i.e., "app-direct-node-02").
...
For more information about the structure of the config file, see the [HAProxy documentation](https://www.haproxy.com/blog/the-four-essential-sections-of-an-haproxy-configuration/).

To copy the config file into the VM, we are going to use SCP protocol. The command below will copy it to your home folder on the VM:
...
To validate the installation, browse to http://_public-ip-of-vm_:8080. Below is a screenshot of what it should look like. The dashboard at this point does not contain much valuable information as all the frontend/backend services we configured are not running yet.

![haproxy](/images/guides/kots/ha-cluster-ha-proxy.png)

### Installing the app manager and deploying an application

The next step is to install the app manager on the first node, in our case, app-direct-node-01. It's important to note that in reality, you could run the installation on any of the three nodes.

To install the app manager, run SSH into the node, and run the one line installation script. You can get this command from the Replicated vendor portal, as shown below.
...
This process will take several minutes as it will install a Kubernetes 'stack' that will include the add-ons defined in the Kubernetes installer, including the app manager. Once it is finished, you should see something similar as shown below:
...
Next, we are going to log in to the admin console to complete the installation and deploy the application. Since this guide assumes you have a level of familiarity with the app manager, we are not going to go into detail on every step to get the application deployed.

![Log-In](/images/guides/kots/ha-cluster-log-in.png)

Log in to the admin console using the address and password from the installation output. After you have logged in using the provided password, you will need to provide a license. The license is generated in the vendor portal and this guide assumes you already know how to do this. There are no options that need to be enabled or turned on in order to support an HA cluster, so if you already have a development license, it should work for this exercise.

![Add-License](/images/guides/kots/ha-cluster-add-license.png)

After you upload your license, it may take a few minutes to load and continue with the deployment.

When the license is loaded, the next window will depend on the application being deployed as this window renders whatever has been codified in the application's [Config.yaml](https://kots.io/reference/v1beta1/config/).
...
To see everything that has been deployed, run SSH to one of the nodes and then run:
...
If you are following along, you should be able to access the application by browsing to http://_ip-address-of-ha-proxy_. The sample app uses flask 'routes' to call various methods to interact with Postgres. To test if it can write to the database, use the  `/sql-check` route, which will check the connection to the default database and list the databases that are available to the 'postgres' user. To write to the database, first use the `/sql-create` route which creates a database (appdirectdb) with a table (tblrecords) in it. Finally, use the `/sql-add` route to add a row to the table with the timestamp. After the database and table are created, use this route to add more records to the database.

To verify that the data is being written to Postgres, use the Postgres client of your choice. The screenshots included in this guide are from pgAdmin with a connection to the database using the load balancer IP address.

Connect using the client of your choice, and retrieve the records by running a query like this:

`SELECT * FROM tblrecords`

You should see records for each time that the `/sql-add` route is accessed. Below is a screenshot of what it looks like in the pgAdmin UI:
...
In the admin console, under Cluster Management you can view the status of the cluster. At this time, it only has one node.
...
Use SSH to access both nodes and run the command by pasting it into the terminal.
...
To verify that the nodes have joined the cluster, you can run the following commands:
...
There are various testing scenarios for an HA cluster. First, we are focusing on the data retention/loss scenario. The sample application is already configured to write to the embedded Postgres StatefulSet.
...
To verify that data is not lost, you will need a Postgres client. You can use the Postgres client of your choice. The screenshots in this guide are from the pgAdmin UI.

To observe how the pods are scheduled as we conduct our test scenarios, use SSH to access Node 1 and run:
...
- The Postgres instance in this application, and the application itself are not configured or set up for High Availability. This test is meant to show the resiliency capabilities of the Kubernetes installer-created cluster as configured through the app manager.
...
To simulate a node becoming unresponsive or simply crashed, you can stop the VM corresponding to the node. The only caveat here is that you are not running the watch command on the node you are planning to take down.

You are not likely not to see much change in the pods during the first five minutes, other than some pods erroring out. After 5 minutes or so, you will start to see pods being terminated and scheduled on the remaining nodes.
