...
* For critical updates that require customers to install them (such as security patches or breaking changes), use the `--required` flag when creating or promoting the release. When a release is marked as required, users cannot skip the upgrade. This flag can be used with both `release create --promote` and `release promote` commands.
