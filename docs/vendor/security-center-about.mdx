...
:::note
The Security Center is Beta and is available to all vendors.
:::
...
The Security Center surfaces image scan results in the Vendor Portal, Enterprise Portal (Classic), and Enterprise Portal (New). This allows customers to review security information for each application version.
...
In the Vendor Portal, you can filter the container image list by this classification. For more information, see [Filter container images by source or scan status](/vendor/security-center-view-vendor-portal#filter-container-images-by-source-or-scan-status) in _View Security Information in Vendor Portal_.
...
1. For each CVE, Security Center includes an identifier, description, severity, affected artifacts, and available fixes.
1. Security Center displays the results of the CVE scan in the Vendor Portal and both Enterprise Portal experiences:
   ...
   - **Enterprise Portal (Classic)**: Customers can view known vulnerabilities, per-image details, and CVE reduction metrics when comparing versions.
   - **Enterprise Portal (New)**: Customers can view known vulnerabilities, per-image details, CVE reduction metrics when comparing versions, and the earliest vendor release that resolves each CVE. When **Allow raw CVE scan downloads in Security Center** is enabled, customers can also download the raw Grype scan JSON for a release. For more information, see [Configure Security Center display settings](/vendor/security-center-enable-customer-access#configure-security-center-display-settings).
...
1. Security Center stores and serves the generated SBOMs through the Replicated Vendor API to make them available for download:
   ...
   - **Enterprise Portal (Classic)**: Customers can download release-specific SBOMs from the **Security Center** tab.
   - **Enterprise Portal (New)**: Customers can download release-specific SBOMs from the Security page.
...
* Version 1.9.0 or later of the [Replicated SDK](/vendor/replicated-sdk-installing#install-the-sdk-as-a-subchart) is required to report CVE information from specific customer instances back to the Vendor Portal. For more information, see [Customer-Specific CVE Information](/vendor/security-center-view-vendor-portal#customer-specific-cve-information).
  * For Helm CLI installations, additionally set the Replicated SDK to [Report All Images](/vendor/replicated-sdk-customizing#report-all-images). This ensures that the Security Center reports all container images observed in the cluster. This setting is enabled by default for Embedded Cluster installations.
