...
1. Security Center displays the results of the CVE scan in the Vendor Portal and both Enterprise Portal experiences:
   ...
   - **Enterprise Portal (New)**: Customers can view known vulnerabilities, the earliest subsequent release where each CVE is no longer detected, per-image details, and CVE reduction metrics when comparing versions.
