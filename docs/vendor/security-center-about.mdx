...
Security Center uses SecureBuild technology to continuously scan your container images with industry-standard open-source tools—[Grype](https://github.com/anchore/grype) for CVE scanning and [Syft](https://github.com/anchore/syft) for SBOM generation. Unlike point-in-time scanning, vulnerabilities are detected continuously. When new CVEs are disclosed, they automatically appear in your Security Center without requiring a new release.

For technical details about the CVE scanning process, SBOM generation, and SecureBuild API integration, see [How Security Center Works](security-center-how-it-works).
