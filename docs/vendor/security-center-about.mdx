...
You can retrieve the Security Center's scan results programmatically with the Replicated Vendor API. The API returns the same Grype scan data that powers the Security Center dashboards, so you can use the results in a CI/CD pipeline, such as gating the promotion of a release from one channel to the next based on the vulnerabilities found in the release's images.

For more information, see [Retrieve scan results with the Vendor API](/vendor/security-center-retrieve-scan-results).
