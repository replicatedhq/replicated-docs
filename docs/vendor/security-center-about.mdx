...
1. Security Center displays the results of the CVE scan in the Vendor Portal and both Enterprise Portal experiences:
   ...
   - **Enterprise Portal (New)**: Customers can view known vulnerabilities, per-image details, CVE reduction metrics when comparing versions, and the earliest vendor release that resolves each CVE. When **Enable raw CVE scan to be downloadable** is enabled, customers can also download the raw Grype scan JSON for a release. For more information, see [Configure Security Center display settings](/vendor/security-center-enable-customer-access#configure-security-center-display-settings).
