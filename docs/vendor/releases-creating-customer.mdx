...
1. For **Assigned channel**, assign the customer to one or more channels. The channel assignments for a customer is determines the application releases that they can install. For more information, see [Channel Assignment](licenses-about#channel-assignment) in _About Customers_. Consider the following guidelines for assigning channels:
   
   * You can select channels that have at least one release.
   
   * <ChangeChannel/>

   * The Vendor Portal prevents you from assigning KOTS and Helm-only customers to channels with releases that those customers are not able to install. For more information, see [Promoting KOTS-Only and Helm-Only Releases](/vendor/releases-about#kots-helm-releases).
