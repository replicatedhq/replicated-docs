...
:::note
Use "First Pull for Selected License Type" rather than "First Pull Only" if customers may start on a trial before converting to a paid license. "First Pull Only" tracks the first-ever pull regardless of license type, so if a customer pulls software on a trial license first, the notification will not fire again after they convert to paid. "First Pull for Selected License Type" tracks milestones per license type, ensuring the notification fires the first time the customer pulls software under a paid license even if they previously pulled on a trial.
:::
