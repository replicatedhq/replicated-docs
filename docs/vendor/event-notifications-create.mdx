...
5. Configure filters for the event type. If filters are available for the selected event type, the filter configuration modal opens automatically. For more information about how filters work, see [Filter Logic](#filter-logic).
...
6. Choose your notification delivery method:
   ...
   - After entering a URL, you can expand the **Webhook payload format** section to preview the exact JSON payload structure your endpoint receives for the selected event type. Click **Copy payload** to copy the format to your clipboard.
