...
1. Select one or more event types to include in the subscription. For more information, see [Event types (Beta)](/reference/event-types).
...
1. (Optional) For each of the event types that you selected, click the filter icon to configure event filters. For more information, see [About filter logic](#filter-logic) on this page.
