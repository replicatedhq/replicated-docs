...
The Custom Metric Threshold Reached event type requires you to configure a metric name, comparison operator, and notification frequency. You can include only one Custom Metric Threshold Reached event per subscription.
...
| Frequency | Yes | Controls how often you receive the notification (see frequency options below) |
