...
Linking to a port-forwarded service from the Admin Console dashboard requires additional configuration. See [Add a Link to Port-Forwarded Services from the Admin Console](#add-link) below.
...
## Add a Link to Port-Forwarded Services from the Admin Console {#add-link}
...
To link to a port-forwarded service, add the same URL to the `spec.ports.applicationURL` field in the KOTS Application custom resource and the `spec.descriptor.links.url` field in the Kubernetes SIG Application custom resource. When the URLs provided in these fields match, KOTS will rewrite the URL during deployment to the hostname used by the browser.

For more information, see [Rewrite URLs Using KOTS](/vendor/admin-console-adding-buttons-links) in _Adding Links to the Dashboard_. 
