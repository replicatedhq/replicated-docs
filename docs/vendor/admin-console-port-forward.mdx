...
Adding ports to the port forward tunnel allows you to port forward application services without needing to manually run the `kubectl port-forward` command. This can be particularly useful when developing and testing KOTS releases, because it provides a quicker way to access an application after installation compared to setting up cluster ingress or adding a load balancer.
...
The [`kubectl kots admin-console`](/reference/kots-cli-admin-console-index) command opens the KOTS port forward tunnel.
...
You can optionally add a link to a port-forwarded service from the Admin Console dashboard. This requires additional configuration in a Kubernetes SIG Application custom resource. For more information, see [Add a Link to a Port-Forwarded Service on the Admin Console Dashboard](#add-link).
