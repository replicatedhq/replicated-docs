import SdkValuesLimitation from "../partials/replicated-sdk/_limitation-config-with-helm-values-only.mdx"
...
You can customize various aspects of the Replicated SDK by setting the `replicated` values in the parent Helm chart's `values.yaml` file. These `replicated` values are automatically injected by the Replicated SDK during installation. For a complete list of supported configuration options for the SDK, see the [`values.yaml`](https://github.com/replicatedhq/replicated-sdk/blob/main/chart/values.yaml) file for the SDK Helm chart in GitHub. For more information about the SDK values, see [Replicated SDK Helm Values](/vendor/replicated-sdk-overview#replicated-sdk-helm-values).

Common use cases for customizing the SDK include adding common labels and pod labels to the Replicated SDK, installing the SDK with custom RBAC settings, and enabling high availability for the SDK.
...
The Replicated SDK creates Kubernetes resources using the name `replicated`. This includes a `replicated` Deployment, Service, ServiceAccount, and Role. You can brand the SDK to make these resources match your product or company branding rather than exposing the `replicated` name to end customers in the cluster. When you brand the SDK, the SDK's telemetry, license enforcement, or API behavior is unchanged.

Use _one_ of the following methods to brand the SDK:

* [Use a Helm alias](#brand-sdk-alias): You can set an `alias` on the SDK dependency in your application's `Chart.yaml`. Use a Helm alias if the SDK is installed as a subchart.
...
When you set a Helm alias, the values key that passes values to the SDK subchart is also rebranded with your alias. For example, if you set the alias `yourcompany-sdk`, then the values key would become `yourcompany-sdk:` instead of `replicated:`. This rebrand doesn't affect how Replicated delivers values to the SDK at install time. For more information, see [Replicated SDK Helm values](/vendor/helm-install-values-schema#replicated-sdk-helm-values).

To brand the SDK with a Helm alias:

1. In your application Helm chart `Chart.yaml` file, add the `alias` field to the SDK dependency. The alias must be a valid Kubernetes resource name that uses lowercase alphanumeric characters and optionally the `-` character.
   ...
   :::note
   You do not need to add a `global.replicated`, `replicated`, or `<alias>` key to your `values.yaml` file for the SDK. The Replicated registry injects these values automatically when a customer pulls the chart. For more information, see [Helm `global.replicated` Values Schema](/vendor/helm-install-values-schema#replicated-sdk-helm-values).
   :::

1. Update dependencies and package the chart into a `.tgz` archive:

   ```
   helm package --dependency-update .
   ```

1. Test your changes by installing the SDK in a development environment. See [Install the SDK as a subchart](replicated-sdk-installing#install-the-sdk-as-a-subchart) in _Install the Replicated SDK_.

   After installing, check that the SDK resources in the cluster use your alias. For example:

   ```
   $ kubectl get deploy --namespace yourapp
   NAME              READY   UP-TO-DATE   AVAILABLE   AGE
   yourapp           1/1     1            1           45s
   yourcompany-sdk   1/1     1            1           45s
   ```

### Brand the SDK with `nameOverride` {#brand-sdk-name-override}

When you use `nameOverride` to brand the SDK, all of the SDK resources are branded to use your override. However, the following values are unchanged:
* The name of the SDK dependency in your `Chart.yaml` file is still `replicated`
* The SDK values key that's injected in your `values.yaml` file is still `replicated`. For more information, see [Replicated SDK Helm values](/vendor/helm-install-values-schema#replicated-sdk-helm-values)

Replicated doesn't recommend setting both a Helm alias and `nameOverride`. If you do set both an alias and `nameOverride`, `nameOverride` takes precedence.

To brand the SDK by setting the `replicated.nameOverride` value:

1. In your application Helm chart `values.yaml` file, set `replicated.nameOverride` to the name you want the SDK resources to use. The value must be a valid Kubernetes resource name that uses lowercase alphanumeric characters and optionally the `-` character.

   ```yaml
   # values.yaml
   replicated:
     nameOverride: yourcompany-sdk
   ```

   :::note
   If you also followed the steps in [Brand the SDK with a Helm alias](#brand-sdk-alias) to set an `alias`, then you need to set `<your-alias>.nameOverride` in the `values.yaml` file instead of `replicated.nameOverride`. For example, if the alias is `yourcompany-sdk`, set `yourcompany-sdk.nameOverride: yourcompany-sdk` in your `values.yaml` file. This is because the alias changes the name of the key.
   :::
...
1. Test your changes by installing the SDK in a development environment. See [Install the SDK as a subchart](replicated-sdk-installing#install-the-sdk-as-a-subchart) in _Install the Replicated SDK_.
   ...
   After installing, check that the SDK resources in the cluster use your override. For example:
...
With the Replicated SDK version 1.19.0 and later, you can run the SDK in read-only mode for the most restrictive RBAC configuration. Read-only mode prevents the SDK from creating or updating any Kubernetes secrets at runtime. This mode is suitable for security-hardened deployments or clusters managed with GitOps.

#### Requirement

Read-only mode requires version 1.19.0 or later of the SDK.

#### Enable read-only mode

To enable read-only mode, set `readOnlyMode` to true in your Helm chart values:
...
#### About the read-only role

The read-only SDK Role excludes `create` and `update` verbs for secrets. It includes only `get` permissions for secrets.

When `readOnlyMode` is enabled, the SDK Helm chart also skips the chart-managed secrets that normally support support bundle collection and metadata enrichment:

* `replicated-support-metadata`
* `<release-name>-supportbundle`

#### About using the SDK API in read-only mode

Read-only mode affects the following SDK features:

* **Chart-managed support bundle secrets**: The SDK Helm chart does not create the `replicated-support-metadata` Secret or the `<release-name>-supportbundle` Secret. This means the chart does not install the support bundle spec Secret and there is no local Secret for support bundle metadata.
* **Instance tags**: You cannot programmatically set instance tags with the SDK API because tag data requires writing to the `replicated-meta-data` secret. The `POST /api/v1/app/instance-tags` endpoint returns `422 Unprocessable Entity`.
* **Support bundle metadata**: You cannot use the SDK API to write enrichment data that the `supportBundleMetadata` collector would otherwise include in a generated support bundle. The `POST` and `PATCH /api/v1/supportbundle/metadata` endpoints return `422 Unprocessable Entity`.
* **Custom app metrics (air-gapped environments)**: In air-gapped environments, the SDK cannot persist custom metrics to secrets for later collection through support bundles. You can still send custom metrics using the `POST` and `PATCH /api/v1/app/custom-metrics` endpoints, but the data is not stored locally.
* **Custom app metrics (online environments)**: The `POST` and `PATCH /api/v1/app/custom-metrics` endpoints continue to work. The SDK sends metrics directly to Replicated APIs but skips persisting them to local secrets. Deleting individual custom metric keys is not available: the `DELETE /api/v1/app/custom-metrics/{metric_name}` endpoint returns `422 Unprocessable Entity`.
* **Integration mock data**: You cannot update mock data in development mode. The `POST /api/v1/integration/mock-data` endpoint returns `422 Unprocessable Entity`.
* **Read endpoints**: All `GET` endpoints work normally.
...
1. During installation, provide the name of the service account that you created by including `--set replicated.serviceAccountName=CUSTOM_SERVICEACCOUNT_NAME`.
   ...
   **Example**:
   ...
   ```
   helm install wordpress oci://proxy.replicated.com/registry/my-app/beta/wordpress --set replicated.serviceAccountName=mycustomserviceaccount
   ```
...
1. During installation, provide the name of the cluster role that you created by including `--set replicated.clusterRole=CUSTOM_CLUSTERROLE_NAME`.
   ...
   **Example**:
   ...
   ```
   helm install wordpress oci://proxy.replicated.com/registry/my-app/beta/wordpress --set replicated.clusterRole=mycustomclusterrole
   ```
