...
The Helm alias changes only the local name that Helm uses to reference the SDK subchart and the resources it creates in the cluster. The alias does not change the SDK's telemetry, license enforcement, and API behavior. Also, when you use a Helm alias, the SDK chart is still pulled from the Replicated registry at `oci://registry.replicated.com/library/replicated`. For information about how to set a custom domain for the Replicated registry, see [Use custom domains](custom-domains-using).
