...
**Requirement:** Read-only mode requires version 1.19.0 or later of the SDK.
...
The SDK Role excludes `create` and `update` verbs for secrets. The Role includes only `get` permissions for secrets, and the Helm chart does not create the `replicated-support-metadata` Secret.

#### Impact on SDK features
...
* **Read endpoints**: All `GET` endpoints continue to work normally.

#### Example error response

Calling an unavailable write endpoint in read-only mode returns a `422` status with the following response body:
