...
With the Replicated SDK version 1.19.0 and later, you can run the SDK in read-only mode. When read-only mode is enabled, the SDK does not create or update any Kubernetes secrets at runtime. This is useful for environments where the SDK should not have write access to the cluster, such as security-hardened deployments or clusters managed with GitOps.
...
* **RBAC is reduced**: The SDK Role does not include `create` or `update` verbs for secrets. Only read (`get`) permissions are granted for secrets.
...
* **Custom app metrics are still synced to Replicated APIs** when the cluster has internet access. However, sync failures are treated as non-fatal and do not cause errors.
...
When a write endpoint is called in read-only mode, the SDK returns a `422` status with the following response body:
