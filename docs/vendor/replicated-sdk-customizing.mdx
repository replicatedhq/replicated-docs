...
When the Replicated SDK is installed as a subchart, it creates Kubernetes resources using the name `replicated`. This includes a `replicated` Deployment, Service, ServiceAccount, and Role. To make these resources match your product or company branding rather than exposing the `replicated` name to end customers in the cluster, you can rename the SDK resources using either of the following methods:

* [Use a Helm alias](#brand-sdk-alias) on the SDK dependency in your application's `Chart.yaml`.
* [Set the `replicated.nameOverride` value](#brand-sdk-name-override) in your application's `values.yaml`.

Both methods produce the same outcome: the SDK's Kubernetes resources are created with the name you choose instead of `replicated`. Neither method changes the SDK's telemetry, license enforcement, or API behavior, and the SDK chart is still pulled from the Replicated registry at `oci://registry.replicated.com/library/replicated`. For information about how to set a custom domain for the Replicated registry, see [Use custom domains](custom-domains-using).

Choose the method that best fits your chart layout. The Helm alias also changes the values key used to pass values to the SDK subchart (for example, `yourcompany-sdk:` instead of `replicated:`), but this does not affect how the registry-injected SDK values are delivered to the SDK at install time. The `nameOverride` value does not change the values key.

Replicated recommends using a Helm alias when the SDK is installed as a subchart. If the SDK is installed as a top-level chart instead of a subchart, use `nameOverride` because the Helm `alias` field is only available on chart dependencies.

Replicated does not recommend using both methods together. If you do set both an alias and `nameOverride`, `nameOverride` takes precedence and determines the names of the SDK resources in the cluster.
...
1. In your application Helm chart `Chart.yaml` file, add the `alias` field to the SDK dependency. The alias must be a valid Kubernetes resource name that uses lowercase alphanumeric characters and optionally the `-` character.
   ...
   For example, to alias the SDK as `yourcompany-sdk`:
...
1. In your application Helm chart `values.yaml` file, set `replicated.nameOverride` to the name you want the SDK resources to use. The value must be a valid Kubernetes resource name that uses lowercase alphanumeric characters and optionally the `-` character.
   ...
   For example, to rename the SDK resources to `yourcompany-sdk`:
   ...
   :::note
   If you also set a Helm `alias` on the SDK dependency, set `nameOverride` under the alias key instead of `replicated`. For example, if the alias is `yourcompany-sdk`, set `yourcompany-sdk.nameOverride` in your `values.yaml` file. This is because the alias changes the values key used to pass values to the SDK subchart. For more information, see [Brand the SDK with a Helm alias](#brand-sdk-alias).
   :::

   With this method, the SDK dependency in your `Chart.yaml` file remains unchanged:
