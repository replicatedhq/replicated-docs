...
The read-only SDK Role excludes `create` and `update` verbs for secrets. It includes only `get` permissions for secrets, and the Helm chart does not create the `replicated-support-metadata` Secret.
