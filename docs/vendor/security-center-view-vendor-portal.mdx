...
The Security Center dashboard displays the Software Bill of Materials (SBOM) for the release in view, including the format, the number of components and dependencies, the generation date, and the tool used. Select **Download SBOM** to download the SBOM as a JSON file.
