...
* On the **CVE details** tab, for each CVE identified:
  ...
  * The highest severity rating across affected images
...
### Filter container images by source or scan status
...
* **Incomplete scans**: Images whose initial scan is queued, in progress, failed, or could not start because the image was not found. Images with incomplete scans can also appear in the application or Replicated Platform image counts.

You can also select **Show only vulnerable images** to limit the list to images that have known vulnerabilities. The **Showing X of Y images** label reports the number of displayed images. Security Center disables the vulnerable-image filter when you select **Incomplete scans** because these images have no current vulnerability data.

When the release has incomplete scans, Security Center shows when it last checked their statuses. Scan statuses do not refresh continuously. Select **Refresh statuses** to retrieve the current status of each incomplete scan.
...
CVE details are available for all current and previously promoted application release versions. To view CVE information for a specific release, go to **Releases > [Release Version] > Security**. This page shows the same container image list, image counts, filters, and SBOM as the [Release Security dashboard](#release-security-dashboard).
...
To view CVE information for a specific customer instance, go to **Customers > [Customer] > [Instance] > Security**. The customer instance page provides the same image counts, source filters, incomplete-scan filter, and vulnerable-image filter as the Release Security dashboard.
