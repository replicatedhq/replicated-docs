...
1. From the `replicated-cli-tutorial` directory, run the following command to lint the application manifest files and ensure that there are no errors in the YAML:
...
1. From the `replicated-cli-tutorial` directory, run the following command to prepare a release with the default settings:
