...
All Compatibility Matrix clusters support tunneling traffic into a `NodePort` service. 
...
% replicated cluster port expose 1e616c55 --port 32456 --protocol http --protocol https
```

:::note
You can expose a node port that does not yet exist. 
...
## GitHub Action
Replicated publishes GitHub actions that will also expose a port. 
