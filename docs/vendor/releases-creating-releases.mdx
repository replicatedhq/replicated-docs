...
You can create a release with one or more Helm charts, Kubernetes manifests, or a combination. You must package a Helm chart to a `.tgz` file to add it to a release. For more information, see [Create a Release with Your Helm Chart](helm-install-release).
...
1. If a drop down is displayed, select one of the following options:
   ...
   * **Create KOTS release**: Create a release with one or more Helm charts, standard Kubernetes manifests, or a combination. Supports installations with the Helm CLI or with Replicated KOTS.
