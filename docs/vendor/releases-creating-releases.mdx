...
1. Add your files to the release:
   * **Helm-only**: For Helm-only releases, in the **Add the Replicated SDK to your Helm Chart** dialog, follow the steps to package your chart and upload a `.tgz` chart archive. For more information, see [Packaging a Helm Chart for a Release](/vendor/helm-install-release).
