...
1. If a drop down is displayed, select one of the following options:
   * **Create Helm-only release**: Create a release that contains one or more Helm charts. Supports installations with the Helm CLI only.
   * **Create KOTS release**: Create a release that can contain Helm charts, standard Kubernetes manifests, or a combination. Supports installations with the Helm CLI or with Replicated KOTS.

   If no drop down is displayed, continue to the next step.  

1. Add your files to the release:
   * For Helm-only releases, in the **Upload Helm Chart** dialog, drag and drop or select a Helm chart `.tgz` file.
   ...
   * For KOTS releases, drag and drop a Helm chart or other application files to the file directory in the YAML editor. You can also click the plus icon to add a new, untitled YAML file. If you add a Helm chart, In the **Select Installation Method** dialog, select **kots.io/v1beta2** and click **OK**. For more information, see [Configuring the HelmChart Custom Resource](helm-native-v2-using).

     <img src="/images/helm-select-install-method.png" alt="select installation method dialog" width="550px"/>

     [View a larger image](/images/helm-select-install-method.png)
...
1. Click **Promote**. In the Promote Release dialog that opens, edit the fields:
   ...
   For more information about the requirements and limitations of each field, see <a href="releases-about#release-properties">Release Properties</a> in <em>About Releases</em>.
