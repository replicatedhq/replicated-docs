...
import ExportAppEnvVariable from "../partials/releases/_export-app-env-variable.mdx"
import VerifyEnvVariables from "../partials/releases/_verify-env-variables.mdx"
...
This procedure shows how to create an application using local application files.

To create and promote a release using the replicated CLI:

1. <ExportAppEnvVariable/>

1. <VerifyEnvVariables/>
...
1. Run one of the following commands to either create your first release or update an existing release.
...
1. Continue to edit, lint, and update the release as needed until you are ready to promote it. Releases cannot be edited after they are promoted.
