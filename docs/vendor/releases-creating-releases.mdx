...
1. Add your files to the release:
   * For Helm-only releases, in the **Upload Helm Chart** dialog, drag and drop or select one or more Helm chart `.tgz` files. For more information, see [Creating a Release with Your Helm Chart](helm-install-release).
   * For KOTS releases, drag and drop a Helm chart or other application files to the file directory in the YAML editor. You can also click the plus icon to add a new, untitled YAML file.
