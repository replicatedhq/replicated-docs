...
You can create a release with one or more Helm charts. Vendors that support Replicated KOTS installations can also create releases that contain standard Kubernetes manifests.

:::note
You must package each Helm chart and its dependencies to a `.tgz` file before you can add it to a release. For more information, see [Packaging a Helm Chart for a Release](helm-install-release).
:::
...
1. If a drop down is displayed, select one of the following options:
   ...
   * **Create KOTS release**: Create a release with one or more Helm charts, standard Kubernetes manifests, or a combination. Supports installations with Replicated KOTS. KOTS releases that contain Helm charts also support installation with Helm.
