...
A release contains one or more Helm charts. Alternatively, vendors that support Replicated KOTS installations can create a release with standard Kubernetes manifests only or use a combination of standard manifests and Helm charts.

:::note
You must package each Helm chart that you want to include in a release and its dependencies to a `.tgz` file before you can add it to the release. For more information, see [Packaging a Helm Chart for a Release](helm-install-release).
:::
