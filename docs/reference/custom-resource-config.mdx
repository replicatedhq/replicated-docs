...
<table>
  <tr>
    <th>Description</th>
    <td><p>The <code>when</code> property denotes configuration groups or items that are displayed on the admin console <strong>Config</strong> page only when a condition evaluates to true. <code>when</code> is a property of both groups and items. For more information about the <code>when</code> property, including requirements and limitations, see <a href="/reference/custom-resource-config#when">`when`</a> above.</p></td>
  </tr>
  <tr>
    <th>Required?</th>
    <td>No</td>
