...
<table>
  <tr>
    <th>Description</th>
    <td><p>The <code>when</code> property denotes items that are displayed on the admin console <strong>Config</strong> page only when a condition evaluates to true. When the condition evaluates to false, the item is not displayed.</p><PropertyWhen/><WhenRequirements/><WhenNote/></td>
  </tr>
  <tr>
    <th>Required?</th>
    <td>No</td>
