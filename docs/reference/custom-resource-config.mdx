...
* `when` accepts string or boolean values, including string or booleans rendered with [KOTS template functions](/reference/template-functions-about)
* To evaluate to true, values compared in the conditional statement of the `when` property must match exactly, without quotes
* `when` can be used only as a property of a group or item. `when` is _not_ an item type and so cannot be used as an option under `select_one`
