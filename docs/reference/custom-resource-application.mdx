...
For Embedded Cluster v2, KOTS existing cluster, and kURL installations, the Application custom resource also lets you configure other aspects of the Admin Console user experience.
...
For Embedded Cluster v3, `additionalImages` is also used to include images in air gap bundles when those images are not directly referenced in your Helm chart templates. This includes images used by preflight checks, such as the Troubleshoot image used to run checks inside the cluster. See [Include preflight images in air gap bundles](/embedded-cluster/v3/embedded-using#preflight-air-gap-images).
...
An array of strings that reference images to exclude from air gap bundles. Supported for Embedded Cluster v2, Embedded Cluster v3, KOTS existing cluster, and kURL installations.
