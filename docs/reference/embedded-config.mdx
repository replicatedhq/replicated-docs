...
You can optionally customize node roles in the Embedded Cluster Config using the `nodeRoles` key. If the `nodeRoles` key is configured, users select a role to assign to a node when it is joined to the cluster. For more information, see [Managing Multi-Node Clusters with Embedded Cluster](/enterprise/embedded-manage-nodes).
...
* (Required) `name`: The name of the role that will be displayed in the Admin Console.
