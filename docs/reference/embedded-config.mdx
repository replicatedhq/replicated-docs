...
You can optionally define node roles in the Embedded Cluster Config. If nodes roles are defined, users access the Admin Console to assign one or more roles to a node when it is joined to the cluster. For more information about assigning roles to nodes, see [Managing Multi-Node Clusters](/enterprise/embedded-manage-nodes).

For multi-node clusters, roles can be useful if you need to assign application workloads to particular nodes. For example, if your application has graphics processing unit (GPU) workloads, you might choose to define a role that is assigned to the node where those workloads run. Or, you could assign a role to a node that is dedicated to running a specific application workoad that requires a lot of resources.

If you do not define node roles, all nodes added to the cluster are controller nodes. Controller nodes run the Kubernetes control plane and can also run other workloads, such as application or Replicated KOTS workloads.

Embedded Cluster supports defining _controller_ and _custom_ node roles. Controller roles can be assigned to controller nodes. Custom roles can be assigned to worker nodes. For more information, see the sections below.
...
You can define custom roles for any non-controller, or _worker_, nodes in the cluster. Worker nodes can run any workloads that are deployed to the cluster by Embedded Cluster, such as application or Replicated KOTS workloads. Unlike controller nodes, worker nodes cannot run the Kubernetes control plane.
