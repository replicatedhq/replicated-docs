...
The format for specifying Helm extensions uses the same k0s Helm extensions format from the k0s configuration. For more information about these fields, see the [k0s documentation](https://docs.k0sproject.io/head/helm-charts/#example). The `version` field is required. Though the latest version of the chart should successfully deploy if you don't specify a version, this will cause problems for upgrades.
