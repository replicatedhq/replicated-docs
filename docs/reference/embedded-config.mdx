...
When `domains.proxyRegistryDomain` and `domains.appServiceDomain` are set, Embedded Cluster uses the domains specified for requests to the given service. Embedded Cluster also passes the domains to KOTS to ensure that KOTS uses the same custom domains.
...
:::note
Before you can configure Embedded Cluster to use custom domains for the proxy registry and app service, first add and verify your domains in the Vendor Portal. For more information, see [Using Custom Domains](/vendor/custom-domains-using).
:::
