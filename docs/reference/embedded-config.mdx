...
By default, Embedded Cluster uses a k0s config that is tested and known to work. In some circumstances, you might want to change the k0s config.
...
For example, you can do the following to enabled WireGuard-based encryption. Note that other configuration might be necessary. See [`spec.network.calico`](https://docs.k0sproject.io/stable/configuration/#specnetworkcalico) in the k0s documentation for more details.
```yaml
apiVersion: embeddedcluster.replicated.com/v1beta1
kind: Config
spec:
