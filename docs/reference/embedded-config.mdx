...
Defining one or more controller roles can be useful in highly-available clusters with three or more controller nodes if you need to assign a specific workload to one of the nodes. For more information about high availability with Embedded Cluster, see [Managing Multi-Node Clusters with Emebdded Cluster](/enterprise/embedded-manage-nodes).
