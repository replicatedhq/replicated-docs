...
You can add `custom` roles that users can assign to any node in the cluster. Each `custom` role that you add must have a `name` and can also have one or more `labels`. See [labels](#labels).
