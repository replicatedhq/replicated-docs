...
### nonexsistent-status-informer-object

<table>
  <tr>
    <th>Description</th>
    <td>Requires that the value of the <code>statusInformers</code> field is an existing Kubernetes object.</td>
  </tr>
  <tr>
    <th>Level</th>
    <td>Warning</td>
  ...
  <tr>
    <th>Applies To</th>
    <td>
      <p>This will detect `statusInformer` entries in a `kots.io/v1beta1` `Application` manifest, but compares those to all manifests in the release.</p>
      <p>Because the linter does not template helm charts during static analysis, it cannot evaulate <code>statusInformer</code> entries for helm-managed resources. If you have configured status informers for your helm workloads, you can ignore <code>nonexistent-status-informer-object</code> warnings for those workloads. If you find these warnings to be noisy, you can use a <a href="https://docs.replicated.com/reference/custom-resource-lintconfig">LintConfig</a> manifest to set them to `info` or `off`.</p>
    </td>
  </tr>
</table>
