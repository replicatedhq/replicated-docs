...
* There must be no whitespace between `repl` and the `{{` delimiter.
* KOTS template functions used in manifests consumed by KOTS must be valid YAML. This is because the YAML manifests are linted before KOTS template functions are rendered.
