...
KOTS template functions are _not_ supported in Helm charts. However, the HelmChart custom resource provides a way to map values rendered by KOTS template functions to Helm charts in your KOTS releases. This allows you to use KOTS template functions to get information about customer environments during installation or upgrade, then access those values from your application without needing to make changes to any Helm charts directly.
