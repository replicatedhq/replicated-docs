...
The HelmChart custom resource provides a way to access values rendered by KOTS template functions from Helm charts in your KOTS releases. This allows you to use KOTS template functions to get information about customer environments during installation or upgrade, then access those values from your application without needing to make changes to any Helm charts directly.
...
For the Config custom resource, KOTS templates each array of items separately. For more information about the syntax of items in the Config custom resource, see [items](/reference/custom-resource-config#items) in _Config_.
