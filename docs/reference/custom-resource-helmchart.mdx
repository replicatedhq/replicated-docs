...
* `true`: KOTS uses Kustomize to modify the chart then repackages the resulting manifests to install. This method is also referred to as "native Helm".
* `false`: Use the Replicated Helm installation method. KOTS renders the Helm templates and deploys them as standard Kubernetes manifests using `kubectl apply`.
