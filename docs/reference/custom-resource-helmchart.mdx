...
For charts deployed with the native Helm method (`useHelmInstall: true`), `weight` determines the order in which the app manager applies the Helm chart. Charts are applied by weight in ascending order, with lower weights applied first.

In the app manager v1.99.0 and later, the `weight` also determines the order in which charts are uninstalled. Charts are uninstalled by weight in descending order, with higher weights uninstalled first. For more information about uninstalling applications, see [remove](kots-cli-remove) in _kots CLI_.

**Supported values:** Positive or negative integers.
**Default:** `0`
