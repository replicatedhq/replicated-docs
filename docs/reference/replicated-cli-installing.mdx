...
* **`REPLICATED_API_TOKEN`**: A service account or user API token generated from a vendor portal team or individual account. The `REPLICATED_API_TOKEN` environment variable has the following use cases:
  ...
  * Optionally set the `REPLICATED_API_TOKEN` environment variable instead of using the `replicated login` command to authorize the Replicated CLI in MacOS or Linux environments.
  ...
  :::note
  The `REPLICATED_API_TOKEN` environment variable takes precedence over authentication profiles. For more information about authentication profiles, see [CLI Authentication Profiles](/reference/cli-profiles).
  :::
...
## Manage Authentication with Profiles
...
For complete information about authentication profiles, including how to create, manage, and switch between profiles, see [CLI Authentication Profiles](/reference/cli-profiles).
