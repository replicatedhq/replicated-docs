...
:::note
This example uses `HasLocalRegistry`, `LocalRegistryHost`, and `LocalRegistryNamespace`, which are not supported in Embedded Cluster v3. For v3, use the `ReplicatedImageName` and `ReplicatedImageRegistry` template functions instead. See [Template Functions for Embedded Cluster](/embedded-cluster/v3/template-functions).
:::
