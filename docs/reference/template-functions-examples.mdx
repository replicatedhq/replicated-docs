...
### Using Variables to Generate TLS Certificates and Keys

One use case for variables with KOTS template functions is when generating default TLS certificates that users can override by providing their own certificate on the admin console **Config** page.

The following [Sprig](http://masterminds.github.io/sprig/) cryptographic and security functions can be used with KOTS template functions to generate certificate authorities (CAs) and signed certificates in JSON format:
* [genCA](https://masterminds.github.io/sprig/crypto.html)
* [genSignedCert](https://masterminds.github.io/sprig/crypto.html)

:::important
Default values are treated as ephemeral. The following certificate chain is recalculated each time the application configuration is modified. Before using this example, be sure that your application can handle updating these parameters dynamically.
:::
