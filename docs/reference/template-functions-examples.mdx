  ...
  values:
    services:
      my-service:
        ...
        # '{{repl ...}}' syntax is used for `type` to improve readability of the if-else statement
...
This section includes an example of using variables with KOTS template functions.
...
The following example shows how to use variables and KOTS template functions in a [`hidden`](/reference/custom-resource-config#hidden) item in the KOTS Config custom resource to provide the required and optional parameters when calling the `genCA` and `genSignedCert` functions. Because KOTS evaluates each item in the Config custom resource separately, template functions used in an item only have access to variables that are declared in the same item. This means that you can create a configuration item with `hidden` set to `true` to evaluate complex templates and render the results.
