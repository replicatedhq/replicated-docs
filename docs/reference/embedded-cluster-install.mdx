...
```
Where:
* `HOST:PORT` is the host and port of the proxy server. When installing behind a proxy, set both `--https-proxy` and `--http-proxy`. If you don't use an HTTP proxy, set `--http-proxy` to the same host and port as your HTTPS proxy server.
