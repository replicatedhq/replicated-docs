...
<table>
  ...
  <tr>
     <td>`--config-values`</td>
     <td>
        <p>Path to the ConfigValues file for the application. Use when automating installation with Emebdded Cluster. For more information about generating and downloading the ConfigValues file for an application, see [Generating a ConfigValues File](/vendor/releases-configvalues).</p>
...
```bash
sudo ./myapp install --license license.yaml --config-values configvalues.yaml
```
