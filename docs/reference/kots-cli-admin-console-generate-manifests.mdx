...
* The `admin-console generate-manifests` command does not accept the [`--strict-security-context`](/reference/kots-cli-install#usage) flag, which deploys KOTS Pods and containers with a strict security context. To generate Admin Console manifests with a strict security context, add the following to the `securityContext` field in the kotsadm Deployment spec:
