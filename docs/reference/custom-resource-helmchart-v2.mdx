...
The HelmChart v2 custom resource supports the native Helm v2 installation. This v2 installation method does a Helm install or upgrade on the vendor’s chart, and bypasses existing Kustomize logic to re-write images, inject pull secrets, and add backup labels and annotations.  As a vendor, you must configure a combination of Replicated and Helm templating to re-write images, inject pull secrets, and add backup labels and annotations.
...
* To support online installations that are configured to push and pull from a private registry (where `disable image push` is _not_ selected), vendors must provide the necessary values in the `builder` field to render the helm chart with all necessary images
