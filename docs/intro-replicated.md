...
The Replicated platform is useful for vendors who have applications that are Helm charts, that use standard Kubernetes manifests, or that use Kubernetes Operators.

Additionally, when you distribute your application with Replicated, you can install your application using Helm, using Replicated installers, or using your own proprietary installer. Replicated KOTS and kURL installers support online and air gap installations.  

Replicated provides the features for managing, getting insights and telemetry, and distributing your apps, including:
...
- Management and development tools, which include the Replicated vendor portal, replicated CLI, Vendor API, SDK, and SDK API
...
- **Builders Plan:** This option supports Helm chart applications and Helm CLI installations, and the optional Replicated SDK features. This plan lets you use all of the Helm features and pick and choose which Replicated SDK features you want to use, depending on your needs. For example, you can use all of the SDK features or just use the compatibility matrix, if all you need is automatic provisioning of test environments. 

    By default, the vendor portal and the insights and telemetry features are built-in to the SDK.
...
Replicated release features help you manage your product lifecycle effectively. You manage releases using the vendor portal, replicated CLI, or the Vendor API.
...
The license service lets you provide custom license entitlements that are signed, synchronized, and verified, such as specifying expiration dates, which features are enabled, and usage limits. Then, the license service integrates those values with your application code to enforce the entitlements. By default, Replicated automatically synchronizes and updates custom entitlement values in the running application.
...
Additionally, if you are using an external private registry, Replicated license entitlements are used to verify customers through the Replicated proxy registry. This eliminates the need to share your registry credentials with customers. For more information, see [Proxy Service](#proxy-service).
...
You can also configure custom domains in the vendor portal for the proxy service and other services that use customer-facing URLs, so your customers see your own branding.

### Compatibility Matrix

The compatibility matrix provides fast, on-demand programmatic access to test your application compatibility in clusters of various Kubernetes distributions and versions using many combinations of customer-representative environments. 
...
Because each vendor's needs are different, you develop the variety of tests that you need, such as smoke tests, policy tests, release tests, and canary tests. Then you run the tests with the compatibility matrix. You can explore a simple configuration, end-to-end configurations, or a broad matrix of combinations. 
...
Preflight checks let you define requirements and dependencies for the cluster where your application is installed. Preflight checks verify a customer environment before they install or upgrade your application, and preflight checks provide clear, immediate feedback to your customer about any missing requirements or incompatibilities. This increases the success rate of installations and helps to prevent support issues.
...
Automatically get telemetry and key insights for the instances of your application running in customer environments. This data provides key performance measurements such as time-to-install, uptime, mean-time-to-resolution (MTTR), upgrade lifecycle, adoption rates, and more.

Instance data helps the update service compile a list of new versions that are available to an instance for upgrade. Adoption metrics help you track how frequently customers are upgrading and which versions they are using. If you see that your customers are on the latest versions, you can reduce the number of older versions that you support and maintain.
