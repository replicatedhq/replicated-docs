...
The CI/CD workflow in the diagram then pushes application images to the vendor's image registry and publishes a release in the Vendor Portal to the "Stable" channel. Customers and internal teams can be assigned to different channels in the Vendor Portal in order to control which application releases they are able to access and install.

Next, the diagram shows how an enterprise customer named "Big Bank" uses the license they were issued to install the application release in their environment. Vendors can create and manage licenses in the Vendor Portal.
