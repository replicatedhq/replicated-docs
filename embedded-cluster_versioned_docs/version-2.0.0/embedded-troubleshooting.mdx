...
:::note
Upgrading to a release that includes the fix does not reclaim the space that is already in use straight away. After the SeaweedFS masters restart, the next maintenance cycle deletes the change log entries and the abandoned uploads, but this only marks the underlying storage as garbage. The masters vacuum a volume automatically when at least 30 percent of that volume is garbage, so a volume with a lower garbage ratio holds the space indefinitely. Use the steps in this section to reclaim the space without waiting.
:::
...
1. Measure the space that SeaweedFS uses on each node. This command is read-only:
...
1. Verify that the space was reclaimed. Repeat the measurement command and compare the values to what you recorded.
