...
1. If you configured the `roles` key to customize node roles, select one or more roles for the node.
    ...
    The Admin Console page refreshes to display the commands for downloading the Embedded Cluster binary, extracting the binary, and joining the node based on the roles you selected. Keep this Admin Console page open for the next steps.
