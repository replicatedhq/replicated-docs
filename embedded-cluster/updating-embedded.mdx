...
1. For the third command, use `upgrade` instead of `install`, then run the command. For example:
   ...
   Replace `APP_SLUG` with your application slug and `LICENSE_FILE` with the path to the license file. Add any other flags you use for this environment. For the full set of flags, see [install](/reference/embedded-cluster-install).
