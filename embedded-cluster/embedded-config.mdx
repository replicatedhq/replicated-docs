...
In the following example, replace `EC_VERSION` with the version of Embedded Cluster to install, including the Kubernetes suffix. For example, `3.12.0-beta.1+k8s-1.36`. Each entry in the [Embedded Cluster v3 release notes](/release-notes/rn-embedded-cluster-v3) lists the full version strings available for that release.
...
Embedded Cluster appends the version of Kubernetes to the version label in the format `+k8s-1.36`. For example, Embedded Cluster `3.12.0-beta.1+k8s-1.36` uses Embedded Cluster `3.12.0-beta.1` and Kubernetes 1.36.

Each version of Embedded Cluster includes specific versions of components like OpenEBS.
For the available versions and the components included in each, see [Embedded Cluster v3 release notes](/release-notes/rn-embedded-cluster-v3).
...
Embedded Cluster installs Helm chart extensions after built-in addons (OpenEBS, registry) and before application charts. This positions extension charts in the infrastructure layer of the [deployment pipeline](embedded-overview#deployment-pipeline). Specify these charts in `spec.extensions.helmCharts`. Each item describes one chart release.
