...
After you have a bundle, upload it to the Vendor Portal for analysis. You can also inspect the bundle locally with sbctl, which lets you run `kubectl` commands against the state of the cluster when the bundle was collected. For more information, see [Inspect Support Bundles](/vendor/support-inspecting-support-bundles).
