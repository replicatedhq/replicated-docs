...
- **As a release-level resource:** Add the Redactor resource to your release alongside your HelmChart custom resources, the same way you did for installations with KOTS. Embedded Cluster publishes the spec into the cluster at install time and applies it to every support bundle. Releases that already define a Redactor need no change.

- **From a Helm chart:** Package the spec in a Secret or ConfigMap in your chart's `templates/` directory. Label the resource `troubleshoot.sh/kind: support-bundle` and put the spec in the `redactor-spec` data key. Embedded Cluster discovers labeled resources in the cluster when it generates a support bundle. This is useful when a redactor belongs to a specific chart, or when you already ship support bundle specs this way.
