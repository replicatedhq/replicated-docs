...
For more information about Embedded Cluster, see [Embedded Cluster Overview](embedded-overview). For information about updating an existing release from Embedded Cluster v2 to v3, see [Upgrade from Embedded Cluster v2](embedded-v3-migrate).
...
1. Add an [Embedded Cluster Config](embedded-config) manifest to the release. At minimum, the Config must specify the Embedded Cluster version to use.
     ...
     Replace `EC_VERSION` with the version to install. For the available versions, see [Embedded Cluster v3 release notes](/release-notes/rn-embedded-cluster-v3).
...
### Whether users can bypass a failed check {#preflight-bypass}

Preflight checks always run. What differs is what happens after a check fails, and that depends on whether the analyzer sets `strict: true` and on whether the installation is running in the installer UI or headless from the CLI.

| Outcome | Installer UI | Headless CLI |
|---|---|---|
| A check fails and the analyzer does not set `strict` | The user is offered **Continue Anyway** and can proceed. | The command exits. Pass `--ignore-host-preflights` or `--ignore-app-preflights` to proceed instead. |
| A check fails and the analyzer sets `strict: true` | The failure is labeled **Required** and **Continue Anyway** is not offered. | The command exits. The ignore flags do not override a required check. |
| The preflight run itself fails or returns no results | No bypass is offered, because there is no result to bypass. | The command exits. The ignore flags do not apply. |

To make a check stop the installation, set `strict: true` on the analyzer. This is the only way to guarantee that a failure blocks installation, because the installer UI offers **Continue Anyway** on any failure that is not marked required. The `--ignore-host-preflights` and `--ignore-app-preflights` flags apply to headless installations and upgrades only, and do not change what the UI offers. For more information about required checks, see [Block installation with required (strict) preflights](/vendor/preflight-defining#strict).

The same behavior applies when checks run during `upgrade` and when host checks run during `node join` and `restore`.

#### What a bypassed application check leaves behind {#preflight-bypass-state}

Host preflight checks run before Embedded Cluster installs anything. Application preflight checks run considerably later, after Kubernetes, the built-in addons, and any Helm chart extensions are installed, and before your application charts are installed. See [Deployment pipeline](embedded-overview#deployment-pipeline).

This means bypassing a failed application check is a different decision from bypassing a failed host check. By the time application checks run, the host already has a running cluster on it. A user who decides not to continue at that point removes the cluster with [reset](embedded-cluster-reset) rather than abandoning the install.
...
- Support bundle specs are separate from preflight specs. Support bundle and Redactor specs continue to use `v1beta2`. See [Add custom redactors](#redactors) on this page.
...
## Add custom redactors {#redactors}

Support bundles generated with Embedded Cluster v3 always apply the Troubleshoot built-in redactors, which hide values that match common patterns for passwords, tokens, and credentials. To scrub additional data, such as API keys or account numbers, add custom redactors with the `troubleshoot.sh/v1beta2 Redactor` resource.

Redactors continue to use `v1beta2`. The `v1beta3` requirement applies only to preflight specs. See [Add preflight checks](#preflights) on this page.

:::note
Custom redactors apply to support bundles only. Preflight checks apply the built-in redactors and ignore Redactor resources.
:::

There are two ways to deliver a Redactor to an Embedded Cluster v3 installation:

- **As a release-level resource:** Add the Redactor resource to your release alongside your HelmChart custom resources, the same way you did for installations with KOTS. Embedded Cluster publishes the spec into the cluster during installation and again during each upgrade. Releases that already define a Redactor need no change.

- **From a Helm chart:** Package the spec in a Secret in your chart's `templates/` directory. Label the Secret `troubleshoot.sh/kind: support-bundle` and put the spec in the `redactor-spec` data key. Embedded Cluster discovers labeled resources in the cluster when it generates a support bundle. This is useful when a redactor belongs to a specific chart, or when you already ship support bundle specs this way. Use a Secret, because a rendered spec can contain the values it redacts.

    ```yaml
    apiVersion: v1
    kind: Secret
    metadata:
      name: my-app-redactors
      labels:
        ...
        spec:
          redactors:
            - name: account numbers
              removals:
                regex:
                  - redactor: '(account_number: )(?P<mask>\d+)'
    ```

You can use both methods in the same release. Embedded Cluster merges the specs and applies all of them.

:::important
Embedded Cluster reads custom redactors from the cluster. If the cluster is unreachable when a customer generates a support bundle, the bundle contains host-level data with the built-in redactors applied only. Do not rely on a custom redactor to keep a value out of every bundle.
:::

### Use template functions in redactors

The template syntax available in a Redactor depends on how you deliver it:

- **Release-level Redactor:** Use Replicated template functions. Embedded Cluster renders the spec against the customer's config values before it publishes the spec to the cluster, so functions such as `repl{{ ConfigOption "..." }}` resolve. This is the same rendering that KOTS applies to release-level Redactor resources, so specs that already use template functions continue to work.

- **Redactor in a Helm chart:** Use Helm template syntax, such as `{{ .Values.exampleValue }}`. Helm renders the resource at install time. Replicated template functions are not supported directly in Helm charts. For more information, see [Supported file types](/reference/template-functions-about#supported-file-types) in _About Template Functions_.

The following release-level example redacts the value that the customer entered for a config item named `api_token`:
...
spec:
  redactors:
    - name: api token
      removals:
        values:
          - 'repl{{ ConfigOption "api_token" }}'
```

:::note
A rendered redactor can contain the resolved value of a password config item. Embedded Cluster stores the rendered release-level spec in a Secret in the `embedded-cluster` namespace.
:::

For the objects and fields supported by the Redactor resource, see [Redactor](/reference/custom-resource-redactor). For more information about redactors, see [Redacting Data](https://troubleshoot.sh/docs/redact/) in the Troubleshoot documentation.
