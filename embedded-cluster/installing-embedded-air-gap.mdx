...
For supported CLI flags, see [install](/reference/embedded-cluster-install).
...
1. On the air-gapped machine, run the `install` command from the dialog. It must include `--airgap-bundle` pointing at your `.airgap` file. For example:
   ...
   Replace `APP_SLUG` with your application slug and use the correct paths for your license and bundle files. Add any other flags you need. For the full set of flags, see [install](/reference/embedded-cluster-install).
...
1. On the air-gapped machine, run `install` with:
   ...
   * `--installer-password` with the password you will use to access the installer later.
   ...
   Replace placeholders with values for your environment. Add any other flags you need. For the full set of flags, see [install](/reference/embedded-cluster-install).
