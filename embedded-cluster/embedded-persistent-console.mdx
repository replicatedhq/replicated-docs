...
| Operation | Persistent console | Command line on a node |
|---|---|---|
| View the installed version and application links | Yes | Yes |
| See available updates and release notes | Yes | Yes |
| Upgrade the application | Yes | Yes |
| Change configuration values | Yes, during an upgrade | Yes |
| Upload an air gap release archive | Yes | Yes |
| Generate and download a support bundle | Yes | Yes |
| Install the application and the console | No | Yes |
| Get a command to join another node | No | Yes |
...
The install and upgrade wizards are separate from the persistent console. Both let a user generate a node join command in the browser during a multi-node installation or upgrade. The command itself still has to be run on the host that is joining, so adding a node always requires command-line access to that host. Removing a node is only available from the command line.
