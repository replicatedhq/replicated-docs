...
The browser experience spans two surfaces. The persistent admin console is the always-on dashboard. The install and upgrade wizards are where configuration, preflight checks, and the install or upgrade run itself happen. Starting an upgrade from the console prepares it and then hands the user to the upgrade wizard, so a user does not have to choose between them.

What matters for an end user without command-line access is therefore whether an operation is available in the browser at all:

| Operation | Browser | Command line on a node |
|---|---|---|
| View the installed version and application links | Yes, in the console | Yes |
...
| Change configuration values | Yes, in the wizard during an install or upgrade | Yes |
| Get a command to join another node | Yes, in the wizard | Yes |
| Install the application and the console | No | Yes |
...
Joining a node is the one case that starts in the browser but cannot finish there. The wizard generates a join command, and that command has to be run on the host that is joining.
