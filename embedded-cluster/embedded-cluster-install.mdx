...
<table>
  ...
  <tr>
    <td>`--api-port`</td>
    <td>int</td>
    <td>Port for the external daemon API used by joining nodes. **Default:** `30081`</td>
  </tr>
  <tr>
    <td>`--cidr`</td>
    <td>string</td>
  ...
  <tr>
    <td>`--roles`</td>
    <td>strings</td>
    <td>Comma-separated role names to assign to this node. Must include the controller role. Defaults to the configured controller role. For more information about defining roles, see [roles](embedded-config#roles) in _Embedded Cluster Config_.</td>
  </tr>
  <tr>
    <td>`--tls-cert`</td>
    <td>string</td>
