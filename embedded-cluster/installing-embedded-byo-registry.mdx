...
The BYO flags follow patch semantics on `upgrade`:
...
* Pass `--registry=""` to disable BYO entirely and revert the cluster to the default registry mode. For online installs, this restores the Replicated proxy registry (or the configured [custom domain](/vendor/custom-domains-using)). For air gap installs, Embedded Cluster redeploys its internal image registry.

When the address changes, Embedded Cluster restarts containerd on each node so the new registry endpoint takes effect. When only credentials, the CA certificate, or the mTLS material change, the containerd registry configuration is hot-reloaded without a restart.

### Enable BYO registry on an existing cluster

To switch a cluster that was installed without BYO registry over to a BYO registry, run `upgrade` with the full set of BYO flags that you would have passed at install time. For example:
...
Before running this command, push all images for the installed release to the BYO registry using the [Registry layout requirements](#registry-layout). The upgrade runs the same BYO image preflight as `install`.

In air gap clusters, switching to BYO does not remove the internal image registry or SeaweedFS from the cluster. They remain in place but are unused while BYO is active.
...
The same pattern applies to `--registry-ca-cert`, `--registry-tls-cert`, and `--registry-tls-key`. Containerd reloads the new material without restarting.

### Switch to a different BYO registry

To move the cluster to a different BYO registry, pass the new address and any credentials or certificates the new registry requires. Any flag you omit keeps its previous value, so explicitly clear fields that no longer apply. For example, if the previous registry used a private CA and the new one does not:
...
Push the release images to the new registry before running the upgrade.
...
Embedded Cluster removes the BYO containerd configuration from each node, along with any CA or mTLS files written for BYO. For online clusters, image pulls return to the Replicated proxy registry (or the configured custom domain). For air gap clusters, the internal image registry is used again; the original internal registry credentials are preserved across the BYO lifecycle, so the revert does not trigger a registry restart or a credentials-mismatch window.
