...
Also, because KOTS is removed, you must include the [Replicated SDK](/vendor/replicated-sdk-overview) in your application to get reporting from application status informers (for example, for notifications and visibility in the Vendor Portal and Enterprise Portal).
...
* Some Replicated template functions are not implemented in Embedded Cluster v3, either because they are discouraged or because they are not yet ported. Contact Replicated if a missing template function blocks your release.
