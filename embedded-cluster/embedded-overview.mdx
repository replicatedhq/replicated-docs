...
You can optionally define node roles in the Embedded Cluster Config. For multi-node clusters, roles can be useful for assigning specific application workloads to nodes. If node roles are defined, users assign one or more roles to a node when it joins the cluster.
