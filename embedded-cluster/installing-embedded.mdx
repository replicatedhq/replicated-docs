...
For supported CLI flags (proxy, network interface, and more), see [install](/reference/embedded-cluster-install).
...
1. For the third command, run `install` with the following flags:
   ...
   * `--installer-password` with the password you will use to access the installer later.
   ...
   Replace `APP_SLUG`, `LICENSE_FILE`, `PATH_TO_CONFIGVALUES`, and `INSTALLER_PASSWORD` with the values for your environment. Add any other flags you need. For the full set of flags, see [install](/reference/embedded-cluster-install).
